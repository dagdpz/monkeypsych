...
esperimentazione = {'choice target-distractor direct saccades horizontal With Sample Presentation'};
...
%%
    ...
    for n_exp = 1:numel(esperimentazione)
     ...
     %% Match-to-sample stimulus: rotation defines the sample & rotationDifference defines the distractor/NonSample   
        ...
        difficultyLevels                    = [5];
        All.rotations                       = 167:All.differenceBetweenSamples:194; %10 different samples
     ...
     % TimeOut & Reward 
        ...
        switch experiment
            ...
            case 'choice target-distractor direct saccades horizontal With Sample Presentation'
                All.instructed_choice_con           = 1;
                All.targets_con                     = 3; % 0 - one peripheral stimulus, 1 - two peripheral stimuli horizontal, 2 - two peripheral stimuli diagonal
                All.shape_con                       = 2;
                All.angle_cases                     = angle_cases_singleOption;
                All.stim_con                        = stim_con_direct;                
                All.target_Right                    = [0,1];
...
%black
task.hnd.tar(1).ringColor = 0;
task.hnd.tar(2).ringColor = 0;
task.hnd.tar(3).ringColor = 0;
task.hnd.cue(1).ringColor = 0;
...
task.timing.msk_time_hold=0.1; %MASK
...
switch Current_con.timing_con
    ...
    case 1 % choice 
        ...
        task.timing.tar_time_to_acquire_hnd     = 3; %0.6
...
switch Current_con.size_con
    ...
    case 2 
        ...
        task.eye.fix.radius     = 45;
        task.eye.cue(1).size    = 0.2;
        task.eye.cue(1).radius  = 45;
        task.eye.tar(1).size    = 0;
        task.eye.tar(1).radius  = 45;
...
task.hnd.tar(3) = [];      
  case 2
  case 3
      %task.hnd.tar(3).size = 2;
      %task.hnd.tar(3).shape.BarSize_L = task.hnd.tar(3).size
      %task.hnd.tar(3).BarSize_W = 
end     
%% STIMULATION timing
