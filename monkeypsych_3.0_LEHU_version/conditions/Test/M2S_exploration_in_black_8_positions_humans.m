...
%experiment='Match to sample saccades random locations';
    ...
    switch experiment
        ...
        case 'Match to sample saccades'
            ...
            N_repetitions                   = 1;        % 1 repetition -> 48 trials
            ...
            All.shape_con                   = [1:4];    % [3,6,10,11];% target and distractor distribution
            %PEST_ON=0;
...
switch Current_con.timing_con
    ...
    case 5 %'Match to sample'
        ...
        task.timing.mem_time_hold               = 0;%früher 0.3; % memory period
        task.timing.mem_time_hold_var           = 0;%früher 0.5; % memory period variance
        ...
        task.timing.tar_time_hold               = 20; % selection time
...
switch Current_con.color_con
    ...
    case 2        
            tar_color=[3 0 0];
    case 3
            tar_color=[100 100 100];            
    case 4
            tar_color=[randi([0,1])*20 0 0];     
end

task.eye.fix.color_dim       = [255 0 0]; 
...
shapes_available=1:numel(all_curv);
shapes_available=shapes_available(shapes_available~=Current_con.shape_con);
shapes_available=Shuffle(shapes_available);
distractor_convexity = all_curv(shapes_available(end));
distractor_side = all_sides{shapes_available(end)};
shapes_available=[Current_con.shape_con, shapes_available(1:2)];
task.convexities=num2cell([all_curv(shapes_available)]);
convex_sides=[all_sides(shapes_available)];
