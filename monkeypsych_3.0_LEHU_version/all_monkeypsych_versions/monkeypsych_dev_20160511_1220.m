...
%             dyn.fix.HT = [dyn.HTeye dyn.HThnd];
        ...
        %% STATE.TAR_ACQ    
            ...
            dyn.tar_time_hold = trial(dyn.trialNumber).task.timing.tar_time_to_acquire_eye;
...
%             dyn.fix.HT = [dyn.HTeye dyn.HThnd];
        ...
        %% STATE.TAR_ACQ_INV 
            ...
            dyn.fixation_target(1) = trial(dyn.trialNumber).task.eye.fix.x;
            dyn.fixation_target(2) = trial(dyn.trialNumber).task.eye.fix.y;
            dyn.tar_time_hold = trial(dyn.trialNumber).task.timing.tar_inv_time_to_acquire_eye;
...
%             trial(dyn.trialNumber).timing.hnd_tar_HT = dyn.tar.HT(2);
    ...
    if isfield(trial(dyn.trialNumber).task, 'correct_choice_target') && dyn.state==STATE.SUCCESS && ...
            isempty(find(trial(dyn.trialNumber).task.correct_choice_target == dyn.target_selected(1)))% dyn.target_selected(1) ~= trial(dyn.trialNumber).task.correct_choice_target % at the moment only for eye targets
...
while true
    ...
    if success,
        ...
        if  trial(dyn.trialNumber).task.eye.tar(dyn.target_selected(1)).x == (dyn.fixation_target(1)) && trial(dyn.trialNumber).task.eye.tar(dyn.target_selected(1)).y == (dyn.fixation_target(2)) % if target_selected is fixation target
            if GetSecs - tEnterState >= dyn.tar_time_hold % only select target if tar_time_to_acquire_eye is over
                dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
                break
            else
                dyn.target_selected(1) = NaN;
