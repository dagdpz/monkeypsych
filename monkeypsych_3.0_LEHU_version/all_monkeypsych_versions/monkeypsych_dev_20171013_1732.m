           ...
           Beeper_PsychPortAudio(SETTINGS.audioPort,1600, 0.5, 0.2);
            ...
            if SETTINGS.GUI
                ...
                if any(~isnan(task.hnd.fix.color_bright))
                    set(dyn.hnd_position_handle,'Color',nanmean([task.hnd.fix.color_bright; 255 255 255],1)/255);
                end
            end
...
if SETTINGS.GUI,
    set(0, 'currentfigure', dyn.GUI_fig_handle);
    cla;
%     dyn.eye_position_handle = line(0,0,'Color','r','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');
%     dyn.hnd_position_handle = line(0,0,'Color','g','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');
    dyn.eye_position_handle = line(0,0,'Color','r','Marker','o','XlimInclude','off','YLimInclude','off');
    dyn.hnd_position_handle = line(0,0,'Color','g','Marker','o','XlimInclude','off','YLimInclude','off');
...
while true
    ...
    if dyn.state == STATE.ITI && KbCheck                                 % check keyboard
        ...
        if any(find(keyCode)==112) % F1
            ...
            answer = dag_inputdlg(prompt,dlg_title,num_lines,def);
            if ~isempty(answer)
