...
%     CalibrationData.foregroundBrightness = int32(1);
%     CalibrationData.backgroundBrightness = int32(1);
...
%     %calllib(SETTINGS.ViewAPIlibrary, 'iV_GetDeviceName')
%     calllib(SETTINGS.ViewAPIlibrary, 'iV_SetupCalibration', pCalibrationData)
%     calllib(SETTINGS.ViewAPIlibrary, 'iV_Calibrate')
%     disp('Validate Calibration')
%     calllib(SETTINGS.ViewAPIlibrary, 'iV_Validate')
    disp('Show Accuracy')
        ...
        case 2 % DPZ
            while true
                ...
                if  bit_state,
                    ...
                    disp(sprintf('Task will start in %f seconds...',SETTINGS.skip_volumes*SETTINGS.TR))
                    break;
                end
            end
    end    
...
%             if SETTINGS.useViewAPI                
%                 % clear recording buffer SMI
%                 calllib(SETTINGS.ViewAPIlibrary, 'iV_ClearRecordingBuffer');
%                 % start recording SMI
%                 calllib(SETTINGS.ViewAPIlibrary, 'iV_StartRecording');
%                 calllib(SETTINGS.ViewAPIlibrary, 'iV_SendImageMessage',['Recording Note: ' num2str(dyn.trialNumber) 'Image']);
%                 
%                 calllib(SETTINGS.ViewAPIlibrary, 'iV_StopRecording');
%                 %calllib(SETTINGS.ViewAPIlibrary, 'iV_SaveData', filename, description, user, ovr);
%                   
%            end
...
%             end
            for n_obj=1:dyn.n_eye_tar
                % comment if fixation spot should be invisible
...
%                 if trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).x ==  trial(dyn.trialNumber).task.(dyn.effector).fix.x && ... % comment if fixation spot should be invisible
...
%             trial(dyn.trialNumber).timing.hnd_tar_HT = dyn.tar.HT(2);
        ...
        %% STATE.CLOSE  
            ...
            if SETTINGS.useVPacq                
                calllib(SETTINGS.ViewAPIlibrary, 'iV_Disconnect')                
                unloadlibrary(SETTINGS.ViewAPIlibrary);
            end
...
%     end
    ...
    if dyn.state==STATE.SUCCESS && ~any(trial(dyn.trialNumber).task.correct_choice_target == dyn.target_selected(dyn.tar_selected_ind)) % for eye and hand targets
        dyn.state=STATE.ABORT;
        %dyn.correct_choice_target = trial(dyn.trialNumber).task.correct_choice_target;
        trial(dyn.trialNumber).completed = 1;
        dyn.completed = 1;
...
else % microstim as digital output
    dyn.pp_reward_value=dyn.pp_reward_value+SETTINGS.pp.value_out_microstim;
    io32(pp.ioObj,pp.address_out_reward,dyn.pp_reward_value); % send microstim trigger/pulse
    tPulseDelivery = GetSecs;
    %WaitSecs(pulse_duration);
    io32(pp.ioObj,pp.address_out_reward,dyn.pp_reward_value-SETTINGS.pp.value_out_microstim); % stop microstim trigger/pulse
end
