...
[SETTINGS.window, SETTINGS.screenSize] = Screen(SETTINGS.whichScreen, 'Openwindow', [],[],[],[],[],3);     % put 'window' on complete screen
...
%             task.eye.fi2.x      =condition(2);%KK
%             task.eye.fi2.y      =condition(3);
...
%             task.hnd.fi2.x      =condition(8);%KK
%             task.hnd.fi2.y      =condition(9);
...
%             task.eye.cuA(2).y   =condition(7);
            ...
            task.eye.cuA(1).x   =condition(12);
            task.hnd.cuA(1).x   =condition(13);
            task.eye.cuA(1).y   =condition(12);
            task.hnd.cuA(1).y   =condition(13);            
...
%             for n_obj=1:dyn.n_hnd_tar %not flexibel!!
%             temp_tar(n_obj).x = trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).x;
%             temp_tar(n_obj).y = trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).y;
%             end

%             for n_obj=1:dyn.n_hnd_ta2 %not flexibel!!
%              trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).x =  trial(dyn.trialNumber).task.(dyn.effector).ta2(n_obj).x;
%              trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).y =  trial(dyn.trialNumber).task.(dyn.effector).ta2(n_obj).y;
%             end
            dyn.duration            = get_state_duration(task,trial,dyn);
            [success,dyn,task]      = acquisition_state(task,par_eye,par_hnd,dyn,trial,IO);
            dyn.stay_condition      = 0;
...
%             for n_obj=1:dyn.n_hnd_tar %not flexibel!!
%             trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).x = temp_tar(n_obj).x ;
%             trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).y = temp_tar(n_obj).y ;
%             end
...
%             if dyn.n_eye_tar>0
%                 for n_obj=1:dyn.n_eye_tar
%                      if ~isnan(dyn.target_selected(1)) && dyn.target_selected(1)==n_obj
%                          par_eye = aux_FillPar(trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).shape, trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).pos, task.eye.ta2(dyn.target_selected(1)).color_bright, ...
%                     trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).ringColor, trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).reward_prob,trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).ringColor2);
%                      else
%                     par_eye(n_obj) = aux_FillPar(trial(dyn.trialNumber).eye.ta2(n_obj).shape, trial(dyn.trialNumber).eye.ta2(n_obj).pos, task.eye.ta2(n_obj).color_dim, ...
...
%             end
        ...
        case {STATE.CUE_ON_AUDITIV}
            ...
            if trial(dyn.trialNumber).task.CueAuditiv == 1
            ...
            elseif trial(dyn.trialNumber).task.CueAuditiv == 0
            end
...
while true
    ...
    if success,
        if  ~isnan(dyn.target_selected(dyn.tar_selected_ind)) && ...
                trial(dyn.trialNumber).task.(dyn.effector).(dyn.tar_struct)(dyn.target_selected(dyn.tar_selected_ind)).x ==  trial(dyn.trialNumber).task.(dyn.effector).fix.x && ...
                trial(dyn.trialNumber).task.(dyn.effector).(dyn.tar_struct)(dyn.target_selected(dyn.tar_selected_ind)).y == trial(dyn.trialNumber).task.(dyn.effector).fix.y && ...
