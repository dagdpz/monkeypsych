...
%                    trial(dyn.trialNumber).hnd.tar(n_obj).ringColor = task.hnd.tar(n_obj).ringColor; %KK ACHTUNG 
...
%             dyn.tar.HT = [dyn.HTeye dyn.HThnd];
        ...
        case {STATE.CUE_ON , STATE.DEL_PER} 
            ...
            for n_obj=1:dyn.n_hnd_cue
                % trial(dyn.trialNumber).hnd.cue(n_obj).ringColor = task.hnd.cue(n_obj).ringColor; %KK ACHTUNG 
        ...
        case STATE.REWARD % reward
            ...
            %trial(dyn.trialNumber).task.hnd.tar(1).shape.rotation 
             %trial(dyn.trialNumber).task.hnd.tar(2).shape.rotation
...
elseif current_state == STATE.CLOSE,
    ...
    switch task.type
        ...
        case 9 % delayed Match-to-sample with backward masking (IN FUTURE with Wagering)
             task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.CUE_ON   ...
                STATE.TAR_ACQ  STATE.TAR_HOL  STATE.SUCCESS  STATE.REWARD  STATE.ITI]; %STATE.MSK_HOL  STATE.MEM_PER
...
% % % % short visual presentation before continuing  %KK ACHTUNG
% if STATE.TAR_ACQ == dyn.state && task.type  ==9
%  obj = 1;
...
switch shape
    ...
    case 'circle_withBar'  %KK
        ...
        Screen('FillOval', SETTINGS.window, par.stimColor, par.stimRect);
        % ones(20,100)
        img = repmat(par.ringColor,20,100);
        texture = Screen('MakeTexture', SETTINGS.window, img);
        Screen('DrawTextures',SETTINGS.window, texture,[], stimRect_Bar, par.shape.rotation);
%  Screen(SETTINGS.window,'Flip');
    case 'circle'
