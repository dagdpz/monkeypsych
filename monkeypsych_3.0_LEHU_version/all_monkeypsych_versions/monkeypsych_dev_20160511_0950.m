...
%         if (task.eye.tar(dyn.target_selected(1)).x == (dyn.fixation_target(1)) && task.eye.tar(dyn.target_selected(1)).y == (dyn.fixation_target(2)) && GetSecs - tEnterState >= task.timing.tar_time_to_acquire_eye)...
%                 || (task.eye.tar(dyn.target_selected(1)).x ~= (dyn.fixation_target(1)) ~= task.eye.tar(dyn.target_selected(1)).y == (dyn.fixation_target(2)))
%             dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
%             break;
%         end
    end
