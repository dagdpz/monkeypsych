...
SETTINGS.dyn.memoryBuffer           = single(nan(SETTINGS.bufferSize,9)); % buffer data before writing to file
...
%% Parallel port, used for reward AND sensors (!)
if SETTINGS.useParallel
    SETTINGS.pp.ioObj = setup_pp;
    io32(SETTINGS.pp.ioObj,SETTINGS.pp.address_out_reward,0); % make sure valve is closed
...
[SETTINGS.window, SETTINGS.screenSize] = Screen(SETTINGS.whichScreen, 'Openwindow');     % put 'window' on complete screen
Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);               % fill whole screen black
Screen(SETTINGS.window,'Flip');

%% Open graphics ("GUIs")
if SETTINGS.GUI
    SETTINGS.GUI_fig_handle=figure('Name','Online GUI','Position',SETTINGS.GUI_coordinates,'Renderer', 'Painters');
    ...
    SETTINGS.eye_position_handle = line(0,0,'Color','r','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');%'background');
    SETTINGS.hnd_position_handle = line(0,0,'Color','g','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');%,'background');
    hold on; drawnow;
    ...
    if SETTINGS.ITI_GUI
        ...
        drawnow;
        set(0, 'currentfigure', SETTINGS.GUI_fig_handle);
    end
end
...
if SETTINGS.interface_with_scanner,
    ...
    SETTINGS.TR             = 2; % s
    SETTINGS.run_volumes	= 10; % number of volumes in one run
    ...
    switch dyn.state
        ...
        %% STATE.INI_TRI
            ...
            if any([task.overriding.radius,task.overriding.positions,task.overriding.type,task.overriding.effector,task.overriding.reach_hand,task.overriding.reward,task.overriding.improvers] == 1),
                ...
                %manual_inputs.eye_tar1_rad          =task.eye.tar(1).radius;
                %manual_inputs.eye_tar2_rad          =task.eye.tar(2).radius;
            ...
            if task.overriding.radius == 1,
                ...
                %task.eye.tar(1).radius          = manual_inputs.eye_tar1_rad;
                %task.eye.tar(2).radius          = manual_inputs.eye_tar2_rad;
            ...
            else % no reward modulation
                
                trial(dyn.trialNumber).reward_size = 2;
                
%                 for t = 1:numel(trial(dyn.trialNumber).eye.tar) % assign values for each target
...
%                 end
%                 
%                 for t = 1:numel(trial(dyn.trialNumber).hnd.tar)
...
%                 end                
                ...
                trial(dyn.trialNumber).hnd.tar(1).ringColor         = [];
                trial(dyn.trialNumber).hnd.tar(1).ringColor2        = [];
                trial(dyn.trialNumber).hnd.tar(2).ringColor         = [];
                trial(dyn.trialNumber).hnd.tar(2).ringColor2        = [];
                trial(dyn.trialNumber).hnd.tar(1).reward            = 2;
                trial(dyn.trialNumber).hnd.tar(1).reward_time       = task.reward.time_neutral;
                trial(dyn.trialNumber).hnd.tar(1).reward_prob       = task.reward.prob_neutral;
                trial(dyn.trialNumber).hnd.tar(2).reward            = 2;
                trial(dyn.trialNumber).hnd.tar(2).reward_time       = task.reward.time_neutral;
                trial(dyn.trialNumber).hnd.tar(2).reward_prob       = task.reward.prob_neutral;
                
                trial(dyn.trialNumber).eye.tar(1).ringColor         = [];
                trial(dyn.trialNumber).eye.tar(1).ringColor2        = [];
                trial(dyn.trialNumber).eye.tar(2).ringColor         = [];
                trial(dyn.trialNumber).eye.tar(2).ringColor2        = [];
                trial(dyn.trialNumber).eye.tar(1).reward            = 2;
                trial(dyn.trialNumber).eye.tar(1).reward_time       = task.reward.time_neutral;
                trial(dyn.trialNumber).eye.tar(1).reward_prob       = task.reward.prob_neutral;
                trial(dyn.trialNumber).eye.tar(2).reward            = 2;
                trial(dyn.trialNumber).eye.tar(2).reward_time       = task.reward.time_neutral;
                trial(dyn.trialNumber).eye.tar(2).reward_prob       = task.reward.prob_neutral;
            ...
            if task.type<5
                if ~task.choice
                    dyn.n_eye_tar=1;
                    dyn.n_hnd_tar=1;
%                 elseif task.choice==1
%                     dyn.n_eye_tar=numel(task.eye.tar); %2;
%                     dyn.n_hnd_tar=numel(task.hnd.tar); %2;
                end
            end
            ...
            if ~task.choice
                ...
                dyn.n_hnd_cue=1;
%             elseif task.choice==1
%                 dyn.n_eye_cue=numel(task.eye.cue); %2;
%                 dyn.n_hnd_cue=numel(task.hnd.cue); %2;
            end
            ...
            if SETTINGS.GUI
                ...
                %if any(~isnan(task.hnd.tar(1).color_bright))
                %set(SETTINGS.hnd_position_handle,'Color',nanmean(task.hnd.tar(1).color_bright,1)/255);
                %end
            end
        ...
        %% STATE.FIX_ACQ     
            ...
            %dyn.fix.RT = [dyn.RTeye dyn.RThnd];
            if success;
                dyn.trialNumberInitiated = dyn.trialNumberInitiated + 1;
                % fprintf('   trial_Initiated %d',dyn.trialNumberInitiated);
            end
...
%             dyn.fix.HT = [dyn.HTeye dyn.HThnd];
...
%             dyn.tar.RT = [dyn.RTeye dyn.RThnd];
...
%             dyn.tar.HT = [dyn.HTeye dyn.HThnd];
            ...
            elseif success==1
                switch trial(dyn.trialNumber).effector
                    case 0
                        if ~(dyn.target_selected(1) ==1) % || (dyn.target_selected(1) ==2 && task.choice))
                            success=false;
                        end
                    case 1
                        if ~(dyn.target_selected(2) ==1) % || (dyn.target_selected(2) ==2 && task.choice))
                            success=false;
                        end
                end
            end
...
%             dyn.fix.HT = [dyn.HTeye dyn.HThnd];
        ...
        %% STATE.TAR_ACQ_INV 
            ...
            eye_tar_color = bg_color;
            hnd_tar_color = bg_color;
            switch trial(dyn.trialNumber).effector
                case 3 % bright hnd fixation spot for dissociated memory saccades
            ...
            for n_obj=1:dyn.n_eye_tar
                ...
                if trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).x ==  trial(dyn.trialNumber).task.(dyn.effector).fix.x && ...
                        trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).y ==  trial(dyn.trialNumber).task.(dyn.effector).fix.y
                    eye_tar_color = trial(dyn.trialNumber).task.(dyn.effector).tar(n_obj).color_dim;
                    bg_color = eye_tar_color;
                end
                ...
                par_eye(n_obj) = aux_FillPar(trial(dyn.trialNumber).eye.tar(n_obj).shape, trial(dyn.trialNumber).eye.tar(n_obj).pos, eye_tar_color, ...
                    bg_color, trial(dyn.trialNumber).eye.tar(n_obj).reward_prob,bg_color);
            end
            for n_obj=1:dyn.n_hnd_tar
                par_hnd(n_obj) = aux_FillPar(trial(dyn.trialNumber).hnd.tar(n_obj).shape, trial(dyn.trialNumber).hnd.tar(n_obj).pos, hnd_tar_color, ...
                    bg_color, trial(dyn.trialNumber).hnd.tar(n_obj).reward_prob,bg_color);
            end
...
%             dyn.tar.RT = [dyn.RTeye dyn.RThnd];
        ...
        %% STATE.TAR_HOL_INV        
            ...
            if ~isnan(dyn.target_selected(1))
                ...
                if trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).x ==  trial(dyn.trialNumber).task.(dyn.effector).fix.x && ...
                    ...
                    eye_tar_color = trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).color_dim;
                    bg_color = eye_tar_color;
                end
                ...
                par_eye = aux_FillPar(trial(dyn.trialNumber).eye.tar(dyn.target_selected(1)).shape, trial(dyn.trialNumber).eye.tar(dyn.target_selected(1)).pos, eye_tar_color, ...
                    bg_color, trial(dyn.trialNumber).eye.tar(dyn.target_selected(1)).reward_prob,bg_color);
            else
                par_eye = struct([]);
            end
            if ~isnan(dyn.target_selected(2))
                par_hnd = aux_FillPar(trial(dyn.trialNumber).hnd.tar(dyn.target_selected(2)).shape, trial(dyn.trialNumber).hnd.tar(dyn.target_selected(2)).pos, hnd_tar_color,...
                    bg_color, trial(dyn.trialNumber).hnd.tar(dyn.target_selected(2)).reward_prob,bg_color);
            else
                par_hnd = struct([]);
            end
...
%             dyn.tar.HT = [dyn.HTeye dyn.HThnd];
        ...
        %% STATE.ABORT    
            ...
            if dyn.previousState==STATE.MAT_HOL || dyn.previousState==STATE.MAT_HOL_MSK % || (numel(trial(dyn.trialNumber).task.correct_choice_target) < dyn.(['n_' dyn.effector '_tar']) && dyn.previousState==STATE.TAR_HOL)
                
                %numel(trial(dyn.trialNumber).task.(dyn.effector).tar) 
                trial(dyn.trialNumber).completed = 1;
            end
            
            % play not nice sound to monkey
            if SETTINGS.useSound && (task.type == 5 || task.type == 6) && trial(dyn.trialNumber).completed == 1,
        ...
        %% STATE.REWARD     
            ...
            if trial(dyn.trialNumber).reward_time > 0,
                ...
                if SETTINGS.useParallel || SETTINGS.useSerial
                    ...
                    %                     aux_DispenseReward(SETTINGS.pp,trial(dyn.trialNumber).reward_time);
        ...
        case STATE.ITI  % ITI
        %% STATE.ITI      
%             trial(dyn.trialNumber).timing.eye_fix_RT = dyn.fix.RT(1);
%             trial(dyn.trialNumber).timing.eye_tar_RT = dyn.tar.RT(1);
%             trial(dyn.trialNumber).timing.eye_fix_HT = dyn.fix.HT(1);
%             trial(dyn.trialNumber).timing.eye_tar_HT = dyn.tar.HT(1);
%             
%             trial(dyn.trialNumber).timing.hnd_fix_RT = dyn.fix.RT(2);
%             trial(dyn.trialNumber).timing.hnd_tar_RT = dyn.tar.RT(2);
%             trial(dyn.trialNumber).timing.hnd_fix_HT = dyn.fix.HT(2);
%             trial(dyn.trialNumber).timing.hnd_tar_HT = dyn.tar.HT(2);
            ...
            dyn.memoryBuffer    = single(nan(SETTINGS.bufferSize,9));    % reset memoryBuffer
            dyn.counterLine     = 0;
        ...
        %% Display
            ...
            if trial(dyn.trialNumber).success
                fprintf(' success (%d %d %d  %s %d)\n', dyn.trialNumber, dyn.trialNumberInitiated, dyn.trialNumberFinished, special_error, special_error_count);
            else
                fprintf(' failure (%d %d %d  %s %d)\n', dyn.trialNumber, dyn.trialNumberInitiated, dyn.trialNumberFinished, special_error, special_error_count);
            end
        ...
        %% STATE.CLOSE  
            % dyn.memoryBuffer = dyn.memoryBuffer(1:dyn.counterLine,:)';
            % fwrite(fidData, dyn.memoryBuffer, 'single');
            % fprintf(['\nNow closing. Data saved to:\n']);disp(outFileData);disp(outFileInfo);
...
%     if dyn.state==STATE.SUCCESS && isempty(find(trial(dyn.trialNumber).task.correct_choice_target == dyn.target_selected(1))) % at the moment only for eye targets
%         dyn.state=STATE.ABORT;
%         dyn.correct_choice_target = trial(dyn.trialNumber).task.correct_choice_target;
%     end
    
    if dyn.state==STATE.SUCCESS && isempty(find(trial(dyn.trialNumber).task.correct_choice_target == dyn.target_selected(dyn.tar_selected_ind))) % for eye and hand targets
        dyn.state=STATE.ABORT;
        dyn.correct_choice_target = trial(dyn.trialNumber).task.correct_choice_target;
        trial(dyn.trialNumber).completed = 1;
    end
    ...
    elseif (~beh_markers(bm_sen1) && ~beh_markers(bm_sen2)),
        ...
        switch dyn.previousState % state in which abort occurred
            ...
            case STATE.TAR_HOL
                ...
                elseif isfield(dyn, 'correct_choice_target')
...
if SETTINGS.GUI
    ...
    if n_obj_eye>0
        ...
        aux_make_targets_invisible(['eye' tag_invisible2 'n1'])
    end
    if n_obj_hnd>0
        ...
        aux_make_targets_invisible(['hnd' tag_invisible2 'n1'])
    end
    drawnow;
end
...
success         = false;
        ...
        case 0 % eye
            ...
            for obj = 1:n_obj_eye
                if aux_IsWithinRadius(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius),
                    ...
                    %fprintf('  RTeye=%4d',round((GetSecs-tEnterState)*1000));
                    %dyn.RTeye=round((GetSecs-tEnterState)*1000);
                end
            end
            hnd_acq = 1;
            ...
            for obj = 1:n_obj_eye
                if aux_IsWithinRadius(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius),
                    ...
                    if isnan(dyn.target_selected(1)),
                        ...
                        dyn.target_selected(2) = NaN; % dummy
                    end
                    %fprintf('  RTeye=%4d',round((GetSecs-tEnterState)*1000));
                    %dyn.RTeye=round((GetSecs-tEnterState)*1000);
                    eye_acq = 1;
    ...
    if SETTINGS.GUI && (tSample > tPreviousEyeHandUpdate + SETTINGS.figure_drawing_interval),
        ...
        aux_DrawEyeHandPos(x_eye, y_eye, x_hnd, y_hnd, task);
    end
    
    dyn.counterLine = dyn.counterLine+1;
    dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2];
        ...
        for obj = 1:n_obj_hnd
            ...
            if ~aux_IsWithinRadius(x_hnd, y_hnd, par_hnd(obj).deg.x, par_hnd(obj).deg.y, par_hnd(obj).deg.radius),
                ...
                elseif GetSecs - tLeaveFixationHnd > task.timing.grace_time_hand
                    ...
                    %dyn.HThnd = round((tLeaveFixationHnd-tEnterState)*1000);
                    %fprintf('  HT=%4d',round((tLeaveFixationHnd-tEnterState)*1000));
        ...
        for obj = 1:n_obj_eye
            ...
            if ~aux_IsWithinRadius(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius),
                ...
                elseif GetSecs - tLeaveFixationEye > task.timing.grace_time_eye
                    ...
                    %fprintf('  HT=%4d',round((tLeaveFixationEye-tEnterState)*1000));
                end
    ...
    if success,
        %         if  trial(dyn.trialNumber).task.eye.tar(dyn.target_selected(1)).x == (dyn.fixation_target(1)) && trial(dyn.trialNumber).task.eye.tar(dyn.target_selected(1)).y == (dyn.fixation_target(2)) % if target_selected is fixation target
        %             if GetSecs - tEnterState >= dyn.duration % only select target if time_to_acquire_eye is over
        %                 dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
        %                 break
        %             else
        %                 dyn.target_selected(1) = NaN;
        ...
        if  ~isnan(dyn.target_selected(dyn.tar_selected_ind)) && ...
                ...
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).y == trial(dyn.trialNumber).task.(dyn.effector).fix.y % if target_selected is fixation target
            if GetSecs - tEnterState >= dyn.duration % only select target if time_to_acquire_eye is over
                dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
                break
            else
...
if task.effector > 0 && task.effector < 5 && all(task.rest_hand) && isempty(dyn.hand_selected)
    success = false;
    beh_markers(bm_sen1) = false;
    beh_markers(bm_sen2) = false;
end
...
while true
            ...
            for obj = start_obj_eye:n_obj_eye %obj = dyn.target_selected(1) %obj = 1:n_obj_eye Change 20140519
                ...
                if ~aux_IsWithinRadius(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius),
                    ...
                    elseif GetSecs - tLeaveFixationEye > task.timing.grace_time_eye
                        ...
                        %dyn.HTeye = round((tLeaveFixationEye-tEnterState)*1000);
                        %fprintf('  HT=%4d',round((tLeaveFixationEye-tEnterState)*1000));
                        % elseif GetSecs - tLeaveFixationEye < task.timing.grace_time_hand
                        % if out for short time, do nothing
                    end
    ...
    if SETTINGS.automatic_offset_update && ~offset_updated && dyn.state==STATE.FIX_HOL && GetSecs > tEnterState + 0.2
        ...
        offset_updated=true;
    elseif dyn.state==STATE.FIX_HOL        
        if SETTINGS.vpx_calibration && ~vpx_calibrated && GetSecs > tEnterState + dyn.duration/2
    ...
    if GetSecs > tEnterState + dyn.duration    	% check time -> holding successful
        success = true;
        %fprintf('  HT=%4d',round((GetSecs-tEnterState)*1000));
        %dyn.HTeye=round((GetSecs-tEnterState)*1000);
        %dyn.HThnd=dyn.HTeye;
        dyn.previousState = dyn.state;
        break
    end
...
% [task pathname] = get_monkey_psych('Test');

% VBLTimestamp = Screen('Flip',SETTINGS.window,[],[],SETTINGS.FlipSyncMode);
% if SETTINGS.useVPacq
%     str = sprintf(repmat('%d\t',1,13),VBLTimestamp,dyn.trialNumber,dyn.trial,dyn.state,zeros(1,8)); % sent marker to VP acquisition
%     vpx_SendCommandString(['dataFile_InsertUserTag 800 "' str '"']) % [800  VBLTimestamp  trialNumber  trial  state  0  0  0  ...]
% end
...
if SETTINGS.GUI,
    set(0, 'currentfigure', SETTINGS.GUI_fig_handle);
    cla;
    SETTINGS.eye_position_handle = line(0,0,'Color','r','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');
    SETTINGS.hnd_position_handle = line(0,0,'Color','g','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');
...
if ~dyn.(['n_' eff '_' pha])==0
    for obj=1:numel(trial(dyn.trialNumber).(eff).(pha))
        if isempty(trial(dyn.trialNumber).(eff).(pha)(obj).pos) % necessary, because the structure is there due to the reward modulation part.
            continue;
        end
        ...
        if obj==1
            plot(x,y,'*','Tag',[eff pha 'n1'],'Visible','off');
        end
        plot(x+x1,y+y1,'r','Tag',[eff pha 'Size'],'Visible','off');
        plot(x+x2,y+y2,'r','Tag',[eff pha 'Radius'],'Visible','off');
%         if (~ismember(trial(dyn.trialNumber).task.type,[5,6]) || strcmp(pha,'cue')) && obj>trial(dyn.trialNumber).choice
%             break;
%         end
        
        if (~ismember(trial(dyn.trialNumber).task.type,[5,6]) || strcmp(pha,'cue')) && trial(dyn.trialNumber).choice == 0 && obj == 1
            break;
        end
...
function aux_DrawEyeHandPos(x_eye,y_eye,x_hnd,y_hnd, task)
% plot eye and hand position
global SETTINGS
if ~isnan(x_eye),
    set(SETTINGS.eye_position_handle,'XData',x_eye,'YData',y_eye);
end
if ~isnan(x_hnd),
    set(SETTINGS.hnd_position_handle,'XData',x_hnd,'YData',y_hnd);
end
...
[x_hnd y_hnd touching] = aux_GetCalibratedHndPos(task);
...
function [x,y, touching] = aux_GetCalibratedHndPos(task)
%% get hand position (from 0 to 1)
global SETTINGS IO

if SETTINGS.touchscreen && SETTINGS.ai
    [x y touching] = get_touch(IO.ai); % in pixels (origin at upper left)
    [x, y] = pix2deg_xy(x, y);
...
if SETTINGS.useParallel
   pp=SETTINGS.pp;
    io32(pp.ioObj,pp.address_out_reward,255);    % open reward valve
    [success,dyn,task]=wait_while_recording_state(task,dyn);
...
stim(3)= deg2pix_withOffset (stim(3),offset_deg);
stim(4)= deg2pix_withOffset (stim(3),offset_deg);
...
function tPulseDelivery = aux_produce_one_trigger(pulse_duration)
if nargin < 1,
...
if ~isdir([SETTINGS.dag_drive filesep 'dag' folder(3:end)])
    mkdir([SETTINGS.dag_drive filesep 'dag'  folder(3:end-8)],folder(end-7:end))
end
if exist('trial','var')
    save([SETTINGS.dag_drive filesep 'dag'  folder(3:end) filesep subfolder],'SETTINGS','task','trial','sequence_indexes');
end
