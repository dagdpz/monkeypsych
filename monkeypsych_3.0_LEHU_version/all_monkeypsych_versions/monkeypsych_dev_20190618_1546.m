...
%             task.eye.ta2(1).radiusShape='circle';
%             task.hnd.ta2(1).radiusShape='circle';
%             task.eye.ta2(2).radiusShape='circle';
%             task.hnd.ta2(2).radiusShape='circle';
%             task.eye.fi2(1).radiusShape='circle';
%             task.hnd.fi2(1).radiusShape='circle';
...
%             task.hnd.cue(2).y   =condition(13);
  ...
  %          trial(dyn.trialNumber).condition      = task.condition;
            ...
            trial(dyn.trialNumber).eye.fix.radiusShape  = task.eye.fix.radiusShape;
            trial(dyn.trialNumber).hnd.fix.radiusShape  = task.hnd.fix.radiusShape;
            if task.type == 10 %KK
            ...
            for n_obj=1:dyn.n_eye_tar % n_obj=1:max(dyn.n_eye_tar,2) % just to keep the same format as in previous versions, at least two targets are always defined
                ...
                trial(dyn.trialNumber).eye.tar(n_obj).radiusShape =task.eye.tar(n_obj).radiusShape;
                if task.effector==4
            ...
            for n_obj=1:dyn.n_eye_ta2 % KK
                ...
                trial(dyn.trialNumber).eye.ta2(n_obj).radiusShape =task.eye.ta2(n_obj).radiusShape;
                if task.effector==4
            ...
            for n_obj=1:dyn.n_hnd_tar % n_obj=1:max(dyn.n_hnd_tar,2) %
                ...
                trial(dyn.trialNumber).hnd.tar(n_obj).radiusShape =task.hnd.tar(n_obj).radiusShape;
                if task.effector==3
            ...
            if task.type == 10
                for n_obj=1:dyn.n_hnd_ta2 % n_obj=1:max(dyn.n_hnd_tar,2) %
                    ...
                    trial(dyn.trialNumber).hnd.ta2(n_obj).radiusShape =task.hnd.ta2(n_obj).radiusShape;
                    if task.effector==3
            ...
            for n_obj=1:dyn.n_eye_cue % n_obj=1:max(dyn.n_eye_cue,2) %
                ...
                trial(dyn.trialNumber).eye.cue(n_obj).radiusShape =task.eye.cue(n_obj).radiusShape;
            end
            for n_obj=1:dyn.n_hnd_cue % n_obj=1:max(dyn.n_hnd_cue,2) %
                ...
                trial(dyn.trialNumber).hnd.cue(n_obj).radiusShape =task.hnd.cue(n_obj).radiusShape;
            end
...
if strcmp(SETTINGS.background_image,'rectangles')    
    ...
    for t=1:numel(task.(strct_for_targets).tar)
        ...
        r= deg2pix_withOffset (task.(strct_for_targets).tar(t).size,offset_deg)*2+2; %+2 because of the frame (1 pixel each direction
        baseRect = [0 0 r r];        
        [x_px, y_px] = deg2pix_xy (task.(strct_for_targets).tar(t).x, task.(strct_for_targets).tar(t).y);
