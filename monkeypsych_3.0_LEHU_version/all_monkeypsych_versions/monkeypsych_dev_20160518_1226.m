...
while true
    ...
    switch dyn.state
        ...
        %% STATE.INI_TRI
            ...
            dyn.fixation_target             = [NaN NaN]; % for task with fixation spot as target
...
%             dyn.fixation_target(1) = trial(dyn.trialNumber).task.eye.fix.x;
%             dyn.fixation_target(2) = trial(dyn.trialNumber).task.eye.fix.y;
            
            %dyn.fixation_target(1) = trial(dyn.trialNumber).task.(dyn.effector).fix.x;
            %dyn.fixation_target(2) = trial(dyn.trialNumber).task.(dyn.effector).fix.y;
...
success         = false;
    ...
    if success,
        ...
        if  ~isnan(dyn.target_selected(dyn.tar_selected_ind)) && ...
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).x ==  trial(dyn.trialNumber).task.(dyn.effector).fix.x && ... (dyn.fixation_target(1))
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).y == trial(dyn.trialNumber).task.(dyn.effector).fix.y % if target_selected is fixation target (dyn.fixation_target(2))
            if GetSecs - tEnterState >= dyn.duration % only select target if time_to_acquire_eye is over
                dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
                break
            else
