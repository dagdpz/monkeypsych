...
Ordered_sequence = repmat([1:N_total_conditions],1,3);
...
task.eye.tar(1).color_dim    = [hue 0 0]; %
task.eye.tar(1).color_bright = [hue 0 0];
task.eye.tar(2).color_dim    = [hue 0 0]; %
task.eye.tar(2).color_bright = [hue 0 0];
...
task.eye.tar(1).radius = 5.5;  % deg
task.eye.tar(2).radius = 5.5;  % deg
...
switch custom_trial_condition
    ...
    case num2cell([(N_stim_conditions*3+1):N_stim_conditions*4])
        task.eye.tar(1).x = -24 + fix_offset;
        task.eye.tar(1).y = fix_y; %% 11.7915; !! was wrong before 20140725
        task.eye.tar(2).x = 24 + fix_offset;
        task.eye.tar(2).y = fix_y; %% 11.7915; !! was wrong before 20140725
