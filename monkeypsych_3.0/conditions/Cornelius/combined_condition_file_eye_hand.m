    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        switch experiment
            ...
            case 'Direct_Reach'
                ...
                task.reward.time_neutral            = [0.3 0.3]; % 0.6s -> 0.2ml per hit
                task.rest_hand                      = [1 1];
                
                All.offset_con                      = 0;
                All.reach_hand_con                  = [1 2]; %1 left 2 right [1 2] %left right random
                All.type_con                        = 2;
                ...
                if ~SETTINGS.take_angles_con
                    ...
                    All.angle_cases                 = [1,3,4,6]; % all 6 targets
                    %All.angle_cases                 = [1,3,4,6]; % 4 targets
                    %All.angle_cases                 = [2,5]; % 2 targets
                end
                All.stim_con                        = 0;
                if PEST_ON==1
                    All.stim_con                    = 0;
                ...
                task.reward.time_neutral            = [0.19 0.19]; %
...
switch Current_con.timing_con
    ...
    case 1 %'direct' 
     ...
     case 2 %'delay' 
        task.rest_sensors_ini_time              = 0.5; % s, time to hold sensor(s) in initialize_trial before trial starts
        task.timing.wait_for_reward             = 0.3;
        task.timing.ITI_success                 = 2; % 2 inter trial interval
        task.timing.ITI_success_var             = 0;
        task.timing.ITI_fail                    = 3; % 3 inter trial interval
        task.timing.ITI_fail_var                = 0;
        task.timing.grace_time_eye              = 0;
        ...
        task.timing.del_time_hold_var           = 0; % 0.5
        task.timing.tar_inv_time_hold           = 0.2;
        task.timing.tar_inv_time_hold_var       = 0.0;
