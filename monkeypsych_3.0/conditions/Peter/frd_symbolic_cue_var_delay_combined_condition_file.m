    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        switch experiment
            ...
            case 'Symbolic_cue_sac_reach'
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                put_back                            = [3 5]; % [3 5] min max - how many trials later error is being put back; if you want exactly 4, use [4 4]
                todays_present_file                 = [pathname(1:end-8) filesep 'shuffled_conditions\shuffled_conditions_' monkey_name 'presented.mat'];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp('Symbolic_cue_sac_reach',esperimentazione)
            ...
            load(todays_present_file);
            
            if dyn.trialNumber > 1 %% exist('dyn','var') && 
            ...
            shuffled_conditions_counter = shuffled_conditions_counter + 1;
                ...
                if trial(end-1).success==0 % previous trial was failure, put it back
                    ...
                    present = [present(1:(shuffled_conditions_counter-1 + err_back - 1),:); ...
                        present(shuffled_conditions_counter-1,:); ...
                        present((shuffled_conditions_counter-1 + err_back):end,:)];
                    
                    %present.err_back(shuffled_conditions_counter + err_back) =err_back; % TABLESSSSS
                    %present(shuffled_conditions_counter + err_back).err_back =err_back; % STTRUCCCTS

                end
                
            else % first trial of run, make safe copy of todays_present_file before this run
                copyfile(todays_present_file,[todays_present_file '.save']);
            end
...
%% using struct present
%       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
%     % choice
%     if strcmp('Symbolic_cue_sac_reach',esperimentazione)
%         
%         % choice
%         if strcmp('choi',present(shuffled_conditions_counter).choice)
%      
%             Current_con.correct_choice_target = 2;
%             
%         elseif strcmp('instr',present(shuffled_conditions_counter).choice)
%             
%             Current_con.correct_choice_target = 1;
%         end
%         
%         % effector
%         if strcmp('hnd',present(shuffled_conditions_counter).eff)
%             
%             Current_con.effector_con = 4;
%             
%         elseif strcmp('eye',present(shuffled_conditions_counter).eff)
%         
%             Current_con.effector_con = 3;
%             
%         end
%         
%         % side  
%         if strcmp('left',present(shuffled_conditions_counter).side)
%             
%             Current_con.exact_excentricity_con_x = -9.5;
%             
%         elseif strcmp('right',present(shuffled_conditions_counter).side)
%         
%             Current_con.exact_excentricity_con_x = 9.5;
%             
%         end
...
switch Current_con.timing_con
    ...
    case 29 % playground
        ...
        task.timing.grace_time_eye              = 0; % time allowed for blinking
        task.timing.grace_time_hand             = 0;
        ...
        % 6 CUE
        task.timing.cue_time_hold               = 1; % .700; 
        task.timing.cue_time_hold_var           = 0; 
        ...
        % 9 GO
        task.timing.tar_inv_time_to_acquire_eye = 1; 
        task.timing.tar_inv_time_to_acquire_hnd = 1;
    ...
    case 31 %Peter's master saccade-reach project
        ...
        task.timing.cue_time_hold               = 0.200; % 
        task.timing.cue_time_hold_var           = 0; % 0
        
        task.timing.mem_time_hold               = present.delay(shuffled_conditions_counter);%2;
        task.timing.mem_time_hold_var           = 0;
        
        task.timing.tar_inv_time_to_acquire_eye = 0.8; %3 % 0.5
        task.timing.tar_inv_time_to_acquire_hnd = 0.8;
...
switch Current_con.size_con
    ...
    case 9 % symbolic cue
        ...
        task.eye.tar(1).radius  = 4.25; %4.25
...
%% COLORS FIXATION
% luminance matching to 130 cd/m^2 revealed green [0 114 0] and red [255 0 0], if measured with the whole screen in that color. 
% [0 255 0] is 300 cd/m^2 and [255 0 0] is 130 cd/m^2

task.eye.fix.color_dim          = [161 0 0]; % color of eye fix before fixated (with both eye and hand)
task.eye.fix.color_bright       = [230 0 0]; % color of eye fix after fixated (with both eye and hand)

task.hnd_right.color_dim_fix     =  [0 70 0]; % color of hand fix before fixated (with both eye and hand)
task.hnd_right.color_bright_fix  =  [0 100 0]; % color of hand fix before fixated (with both eye and hand)
...
% COLOR OF THE CUES
task.eye.cue(1).color_dim       = [230 0 0]; %%%%%%%%% COLOR OF EYE CUE
task.hnd_right.color_cue        = [0 100 0]; %%%%%%%%%%%% COLOR OF THE HAND CUE
...
if strcmp('Symbolic_cue_sac_reach',esperimentazione)
    ...
    save(todays_present_file,'present','shuffled_conditions_counter');
end
