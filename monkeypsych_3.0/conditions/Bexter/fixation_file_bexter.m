...
%         pool_of_angles                      = [40,0,320,220,180,140]; %[20,0,340,200,180,160];
        ...
        fix_eye_y                           = -8;
        fix_hnd_y                           = -15;
        pool_of_angles                      = [90]; %[20,0,340,200,180,160];
%         fix_eye_y                           = -2;
%         fix_hnd_y                           = -5;
%         pool_of_angles                      = [20,0,340,160,180,200];
        
        
        task.reward.time_neutral            = [0.25 0.25];  %[0.25 0.25]; % 0.6s -> 0.2ml per hit
        ...
        switch experiment
            ...
            case 'calibration'
              ...
              force_conditions                    = 2;
                ...
                task.reward.time_neutral            = [0.35 0.35]; % 0.2s -> 0.2ml per hit
                reward_time                         =  0.4  ;    %0.24;
                task.rest_hand                      = [0 0]; % which hands on the sensor to start the trial
                All.rest_hands_con                  = 0;
               
                All.reward_sound_con                = 0;
                All.offset_con                      = 0;
                All.reach_hand_con                  = 1; % 1 left 2 right hand
                %% IMPORTANT
                All.type_con                        = 12; %Auditive Task with specific state
                All.effector_con                    = 0; %6
                All.CueOnAuditiv                    = ones(1, 10); % [1,2]-> 50%
                All.Percentage_CueAuditiv           = 0.9;
                All.Timing_ITI                      = 0.5;
                %%
                All.timing_con                      = 2;
                All.size_con                        =3;
                All.instructed_choice_con           = [0]; %  0 instr  1 choice
                All.var_x                           = 0;
                All.var_y                           = 0;
                ...
                if ~SETTINGS.take_angles_con
                    ...
                    All.excentricities              = [12]; % 16  10
                    All.angle_cases                 = [1]; %1,2,3,4,5,6
                end
                All.stim_con                        = 0;
                if PEST_ON==1
                    All.stim_con                    = 0;
    ...
    if All.type_con == 12
        IndCueAuditiv = find(~cellfun(@isempty, strfind(all_fieldnames, 'CueOnAuditiv')));
         IndReward = find(~cellfun(@isempty, strfind(all_fieldnames, 'reward_con')));
         IndTimingITI= find(~cellfun(@isempty, strfind(all_fieldnames, 'Timing_ITI')));

        En = length(All.CueOnAuditiv);
        St = 1;
        for ind_sequence_matrix = 1: size(sequence_matrix,2)/length(All.CueOnAuditiv)
            sequence_matrix(IndCueAuditiv,St:En)  = randsample([ones(round((All.Percentage_CueAuditiv)*length(All.CueOnAuditiv)),1)', zeros(round((1-All.Percentage_CueAuditiv)*length(All.CueOnAuditiv)),1)'], length(All.CueOnAuditiv));
            sequence_matrix(IndReward,St:En)      = [zeros(round((All.Percentage_CueAuditiv)*length(All.CueOnAuditiv)),1)', ones(round((1-All.Percentage_CueAuditiv)*length(All.CueOnAuditiv)),1)'];
            sequence_matrix(IndTimingITI,St:En)   = randsample(0.5: 0.005:0.5450, length(All.CueOnAuditiv));

            En = En+ length(All.CueOnAuditiv);
            St = St+ length(All.CueOnAuditiv);
        end
    end
...
switch Current_con.timing_con
   ...
   case 1 % Fixation
        ...
        task.timing.wait_for_reward             = 0.8;   %0.3
        task.timing.ITI_success                 = 0.5; % 2    3 inter trial interval
        task.timing.ITI_success_var             = 0;
        task.timing.ITI_fail                    = 0.8; % 3 inter trial interval
        task.timing.ITI_fail_var                = 0;
        task.timing.ITI_incorrect_completed     = 1;        %2;
        task.timing.fix_time_to_acquire_hnd     = 3;%1.5
        task.timing.fix_time_hold               = 0.6; %0.8 
        task.timing.fix_time_hold_var           = 0; % 0
        task.timing.grace_time_eye              = 0;
        task.timing.grace_time_hand             = 0;
        task.timing.cue_time_hold               = 0.15; % 0.28
        task.timing.cue_time_hold_var           = 0; % 0
        task.timing.del_time_hold               = 0; % 1
        task.timing.del_time_hold_var           = 0; % 0          0.5
   
        
    case 2 % Auditiv Task
        
       % task.rest_sensors_ini_time              = 0.5; % s, time to hold sensor(s) in initialize_trial before trial starts
        task.timing.wait_for_reward             = 0.05;
        task.timing.ITI_success                 = Current_con.Timing_ITI; % 3 inter trial interval
        task.timing.ITI_success_var             = 0;
        ...
        task.timing.fix_time_to_acquire_eye     = 0.05;
...
switch Current_con.size_con
  ...
  case 1 %'fixation'
        ...
        task.hnd.fix.size       = 3;         %4;
        task.hnd.tar(1).size    = 14;       %10;
        task.hnd.tar(1).radius  = 14;       %10;
    ...
    case 2 %'delay'
        ...
        task.hnd.fix.radius     = 6;        %8;
        task.hnd.fix.size       = 3;        %4;
        task.hnd.tar(1).size    = 12;        %4;
        task.hnd.tar(1).radius  = 12 ;       %9;
        
   case 3 %'rest'
        task.eye.fix.size       = 0;
        task.eye.fix.radius     = 3000;
        task.eye.tar(1).size    = 0;
        task.eye.tar(1).radius  = 3000;
...
%% COLORS
        ...
        task.hnd_right.color_cue_dim      = [255 255 255];
        task.hnd_left.color_cue_dim     = [255 0 0];
...
if randomize_colors
    rand_colors = rand(1);
    if rand_colors < 0.66
        task.hnd_right.color_cue    = [255 255 255];
        task.hnd_left.color_cue     = [255 255 255];
    else
        task.hnd_right.color_cue    = [255 0 0];
        task.hnd_left.color_cue     = [255 0 0];
    end
end
