    ...
    switch dyn.state
        ...
        case STATE.INI_TRI
            ...
            if ~isempty(task.custom_conditions)
                %if ~SETTINGS.interface_with_scanner || dyn.trialNumber > 1, % IK 20191212
                    run(task.custom_conditions);
               % end
...
while true
    ...
    elseif dyn.state==STATE.FIX_HOL || dyn.state==STATE.FIX_ACQ% || dyn.state==STATE.ITI
        ...
        if keyIsDown
            if SETTINGS.vpx_calibration && any(find(keyCode)==119) % F9
                vpx_SendCommandString(['calibration_snap',blanks(1),num2str(trial(dyn.trialNumber).vpx_calibration_point)]);
            elseif any(find(keyCode)==115) % && isfield(par_eye,'deg')% F4 KK (PN: no fix necessary - commented out)
