...
if SETTINGS.TextFeedback
    Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);    
    displaybackground(SETTINGS);
    Screen('DrawText',  SETTINGS.window, dyn.abort_message, SETTINGS.screen_w_pix/2-numel(dyn.abort_message)*5.5,SETTINGS.screen_h_pix/2, [255 255 255] ,SETTINGS.BG_COLOR); % text position
    Screen(SETTINGS.window,'Flip');
    tempdyn=dyn;
...
elseif SETTINGS.UseMouseAsTouch  % use mouse input instead of touchscreen
    ...
    if button(1)
        ...
        y = atan(y_M/task.vd)*180/pi;
        touching = button(1);
        %ShowCursor(8, SETTINGS.window, 1); % 1 - crosshair, 2 - index finger, 3 - four arrows, 4 - up/down arrow, 5 - left/right arrow, 6 - sand clock, 7 - stop sign, 8 - default arrow (from Windows)
    else
        x = nan;
        y = nan;
        touching = 0;
...
function displaybackground(SETTINGS)

if strcmp(SETTINGS.background_image,'rectangles')
    
    targetsize = 3.5; %actually just the "radius" of the square, so half the length
    visualecc = 9.5;
    distancetoscreen = 89;
    
    targetsize_px = ((SETTINGS.screen_w_pix/SETTINGS.screen_w_cm)* distancetoscreen) *tan(targetsize*2 /2*pi/180) *2;
    visualecc_px = (((SETTINGS.screen_w_pix/SETTINGS.screen_w_cm)* distancetoscreen) *tan(visualecc    /2*pi/180) *2) + 4; % +4 to fit squares perfectly
    %r = 488; %600 768
    r = targetsize_px;
    baseRect = [0 0 r r];
