...
%             for n_obj=1:dyn.n_eye_cuA % n_obj=1:max(dyn.n_eye_cue,2) %
%                 trial(dyn.trialNumber).eye.cuA(n_obj).pos   = [task.eye.cuA(n_obj).x task.eye.cuA(n_obj).y task.eye.cuA(n_obj).size  task.eye.cuA(n_obj).radius 0];
%                 trial(dyn.trialNumber).eye.cuA(n_obj).shape = task.eye.cuA(n_obj).shape;
%             end
%             for n_obj=1:dyn.n_hnd_cue % n_obj=1:max(dyn.n_hnd_cue,2) %
%                 trial(dyn.trialNumber).hnd.cuA(n_obj).pos   = [task.hnd.cuA(n_obj).x task.hnd.cuA(n_obj).y task.hnd.cuA(n_obj).size  task.hnd.cuA(n_obj).radius 1];
%                 trial(dyn.trialNumber).hnd.cuA(n_obj).shape = task.hnd.cuA(n_obj).shape;
%             end
...
%             dyn.tar.HT = [dyn.HTeye dyn.HThnd];
        case STATE.TA2_ACQ  % target acquisition
            ...
            if all(isnan(dyn.target_selected)) % still no target has been selected (e.g. in task.type = 2)
                ...
                if ~isnan(dyn.target_selected(1))
                    par_eye = aux_FillPar(trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).shape, trial(dyn.trialNumber).eye.ta2(dyn.target_selected(1)).pos, task.eye.tar(dyn.target_selected(1)).color_dim, ...
                        trial(dyn.trialNumber).eye.tar(dyn.target_selected(1)).ringColor, trial(dyn.trialNumber).eye.tar(dyn.target_selected(1)).reward_prob,trial(dyn.trialNumber).eye.tar(dyn.target_selected(1)).ringColor2);
                else
                    par_eye = struct([]);
                end
        ...
        case STATE.TA2_HOL %  target holding_with_change_of_color
            ...
            if ~isnan(dyn.target_selected(2))
                par_hnd = aux_FillPar(trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).shape, trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).pos, task.hnd.ta2(dyn.target_selected(2)).color_bright,...
                    trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).ringColor, trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).reward_prob,trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).ringColor2);
            elseif dyn.n_hnd_tar>0
                for n_obj=1:dyn.n_hnd_tar
                    par_hnd(n_obj) = aux_FillPar(trial(dyn.trialNumber).hnd.ta2(n_obj).shape, trial(dyn.trialNumber).hnd.ta2(n_obj).pos, task.hnd.fix(1).color_bright, ...
                        trial(dyn.trialNumber).hnd.ta2(n_obj).ringColor, trial(dyn.trialNumber).hnd.ta2(n_obj).reward_prob,trial(dyn.trialNumber).hnd.ta2(n_obj).ringColor2);
    ...
    switch task.type
        ...
        case 9 % delayed Match-to-sample with backward masking (IN FUTURE with Wagering)
             task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.CUE_ON  STATE.MSK_HOL  STATE.MEM_PER...
                STATE.TAR_ACQ  STATE.TAR_HOL  STATE.SUCCESS  STATE.REWARD  STATE.ITI];
       case 10 % delayed Match-to-sample with backward masking (IN FUTURE with Wagering)
             task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.CUE_ON  STATE.MSK_HOL  STATE.MEM_PER...
                STATE.TAR_ACQ  STATE.TAR_HOL STATE.CUE_ON_AUDITIV STATE.TA2_ACQ  STATE.TA2_HOL  STATE.SUCCESS  STATE.REWARD  STATE.ITI];
