...
while true
    ...
    switch task.effector,
        case 0 % eye
            for obj = 1:n_obj_eye
                if aux_Radius(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius),
                    success = true;
...
global SETTINGS
is_within = 0;
if SETTINGS.Radius_square
    if (norm(cross([x0 - x; y0 - y;0],[x0 - (x0 - r); y0 - (y0 + r);0])) + norm(cross([x0 - x; y0 - y;0],[x0 - (x0 + r); y0 - (y0 + r);0]))) < norm([x0 - (x0 - r); y0 - (y0 + r);0])
        is_within = 1;
    end
