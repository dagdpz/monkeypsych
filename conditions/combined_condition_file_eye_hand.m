...
%           esperimentazione        = {'calibration'};
    %     esperimentazione        = {'Baseline stim project direct saccades'};
    %         esperimentazione        = {'Baseline stim project memory saccades'};
    ...
    %     esperimentazione        = {'Memory diss_saccade'};
    
    %             esperimentazione        = {'Direct saccades'};
    %             esperimentazione        = {'Direct free_gaze'};
    %         esperimentazione        = {'Direct joint'};
    %             esperimentazione        = {'Direct diss_reach'};
    %         esperimentazione        = {'Direct diss_saccade'};
    
    %         esperimentazione        = {'Delay joint'};
    %              esperimentazione        = {'Delay saccades'};
    %
    %
...
%         esperimentazione        = {'Delay free_gaze', 'Delay diss_saccade', 'Delay diss_reach'};
%         esperimentazione        = {'Direct saccades'};
%     esperimentazione        = {'Direct free_gaze old'};
   esperimentazione        = {'Direct saccades' 'Direct free_gaze', 'Direct diss_saccade', 'Direct diss_reach'};
%     esperimentazione        = {'Direct diss_reach'};
% %     

add_4_deg_randomization = 1;
    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        All = struct('angle_cases',0,'instructed_choice_con',0,'type_con',0,'effector_con',0,'reach_hand_con',0,'excentricities',0,'stim_con',0,'timing_con',0,'size_con',0,...
            'tar_dis_con',0,'mat_dis_con',0,'cue_pos_con',0,'shape_con',0,'offset_con',0,'invert_con',0,'exact_excentricity_con_x',NaN,'exact_excentricity_con_y',NaN,'var_x',0,'var_y',0);
        ...
        switch experiment
            ...
            case 'Baseline stim project direct saccades'
                ...
                task.reward.time_neutral            = [0.12 0.12];   % 0.45s -> 0.15ml per hit
                task.rest_hand                      = [0 0];
            ...
            case 'Memory joint'
                ...
                task.reward.time_neutral            = [0.17 0.17]; % 0.6s -> 0.2ml per hit
                task.rest_hand                      = [1 1];
...
%                 All.timing_con                      = 31;
%                 All.size_con                        = 20;
%                 All.instructed_choice_con           = [0 1 1];
%                 All.var_x                          = 0; 
% All.var_y                         = 0;
% 
%                 if ~SETTINGS.take_angles_con
...
%                 
            ...
            case 'Baseline free gaze reaches'
                SETTINGS.check_motion_jaw           = 1;
                SETTINGS.take_angles_con            = 1;
            ...
            case 'RF mapping memory reaches'
                ...
                if ~SETTINGS.take_angles_con
                    All.exact_excentricity_con_x    = [-12 12];
                    All.exact_excentricity_con_y    = [20 0 -20];
                else
                    All.excentricities              = 12;
                    All.angle_cases                 = [1,2,3,4,5,6];
            ...
            case 'RF mapping memory saccades'
                ...
                if ~SETTINGS.take_angles_con
                    All.exact_excentricity_con_x    = [-24 24];
                    All.exact_excentricity_con_y    = [20 0 -20];
                else
                    All.excentricities              = 24;
                    All.angle_cases                 = [1,2,3,4,5,6];
...
switch Current_con.timing_con
    ...
    case 32 %'memory learning'
        ...
        task.timing.tar_time_to_acquire_hnd     = 2;
        task.timing.tar_inv_time_to_acquire_hnd = 2;
        task.timing.fix_time_to_acquire_eye     = 0.5;
        task.timing.tar_time_to_acquire_eye     = 0.5;
        task.timing.tar_inv_time_to_acquire_eye = 0.5; %3
...
%         task.eye.fix.size       = 0.5;
%         task.eye.fix.radius     = 6;
%         task.eye.tar(1).size    = 4;
%         task.eye.tar(1).radius  = 8.5;
%         
%         task.hnd.fix.radius     = 4;
%         task.hnd.fix.size       = 4;
%         task.hnd.tar(1).size    = 4;
...
tar_dis_2x = - tar_dis_x - Current_con.var_x;
