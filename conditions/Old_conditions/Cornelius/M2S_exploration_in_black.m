...
%experiment='Match to sample saccades random locations';
    ...
    switch experiment
        ...
        case 'Match to sample saccades'
            ...
            force_conditions                = 2;
            multiple_targets_per_trial      = 0; % 0 for triangle task
            ...
            %All.tar_pos_con                 = 99;
            All.excentricities              = [12]; % & 20! the eccentricity is randomly chosen to be 12 or 20
            All.angle_cases                 = [1];
            All.tar_dis_con                 = 0; 
...
% RANDOM LOCATIONS
%             case 'Match to sample saccades random locations'
%             
%             SETTINGS.GUI_in_acquisition     = 1;
%             force_conditions                = 3;
...
%             All.size_con                    = 5;
%             % CUE NUMBER
%             All.instructed_choice_con       = 0; % 1 is two cues, 0 is one cue
%             %All.tar_pos_con                 = 99;
%             All.excentricities              = [10,20]; % the two eccentricities of the close and far triangles
%             All.angle_cases                 = [1];
%             All.tar_dis_con                 = 1:2; 
%             All.mat_dis_con                 = 7:8;      % triangle pointing randomly left or right
%             All.cue_pos_con                 = 1:2;      % right and left cue
%             All.shape_con                   = 1:8;    % target and distractor distribution
%             %PEST_ON=0;
%             
%             % THINGS TO CHANGE
%             adjusted_curvature              = -0.6;      % change curvature here
%             % TIMING
%             % change timing for training: line 270
%             
%             % make red dots to cue the position: line 411
...
switch Current_con.timing_con
    ...
    case 5 %'Match to sample'
        ...
        task.timing.tar_time_to_acquire_eye     = 15;  % time to explore the black screen; 12 for the experiment
        task.timing.tar_inv_time_to_acquire_eye = 15;  
        task.timing.fix_time_hold               = 0.2; % fixation time
        task.timing.fix_time_hold_var           = 0.2;
        task.timing.cue_time_hold               = 0.5; % cue time
        ...
        task.timing.tar_time_hold               = 0.8; % selection time
...
%         ex_LR=ex_LR+(randi([0,1])*ex_LR*2/3);
%     end

    % placeholder variables for all 6 positions
    phii=60;
left_top = [-ex_LR*cos(phii*pi/180),fix_height+ex_LR*sin(phii*pi/180)];
left_mid = [-ex_LR,fix_height];
left_bot = [-ex_LR*cos(phii*pi/180),fix_height-ex_LR*sin(phii*pi/180)];
right_top = [ex_LR*cos(phii*pi/180),fix_height+ex_LR*sin(phii*pi/180)];
right_mid = [ex_LR,fix_height];
right_bot = [ex_LR*cos(phii*pi/180),fix_height-ex_LR*sin(phii*pi/180)];


if     multiple_targets_per_trial == 1; % 18 positions for match to sample saccades(!)
        ex_LR = Current_con.excentricities;
        All_positions_left  = Shuffle({left_top, left_mid, left_bot});
        All_positions_right = Shuffle({right_top, right_mid, right_bot});
...
switch Current_con.color_con
    ...
    case 4
            ...
            if ~tar_color(1)
                task.reward.time_neutral        = task.reward.time_neutral + 0.05;
            end
end
