...
%              experiment='calibration';
    %     experiment='Direct saccades flaffi';
        experiment='Memory saccades flaffi';
%     experiment='Direct joint movements';
    
    


%     experiment='Direct dissociated saccades flaffy'; %<------- 20151106 :)
    
    
    
    
%        experiment='Learning memory reaches';
%         experiment='Learning memory lini';
    
%             experiment='Memory lini';
    ...
    All=struct('angle_cases',0,'instructed_choice_con',0,'type_con',0,'effector_con',0,'reach_hand_con',0,'excentricities',0,'stim_con',0,'timing_con',0,'size_con',0,...
        'tar_dis_con',0,'mat_dis_con',0,'cue_pos_con',0,'shape_con',0,'color_con',1);
    ...
    switch experiment
        ...
        case 'Learning memory reaches'
            SETTINGS.check_motion_jaw       = 0;
            force_conditions                = 1; % back to the pool, unsuccessful target would reappear at some point
            N_repetitions                   = 10; % how many time each condition will appear
            ...
            All.color_con                   = [2]; % 1 bright target, 2 dim learning target
            All.reach_hand_con              = [1,2]; % 1 left, 2 right
            All.type_con                    = 3; % 2.5 learning memory, 2 direct
            All.effector_con                = 1; % free gaze reach
            All.timing_con                  = 32; % 32 learning memory, 2 direct
            All.size_con                    = 3; % 2 reach sizes, 3 for bigger targets
            All.instructed_choice_con       = [0,1]; % 0 instructed, 1 choice
            All.excentricities              = [12,24]; % either 12 or [12 24]
            All.angle_cases                 = [1,2,3,4,5,6]; % which targets to present depending on pool of angles
            All.stim_con                    = [0];
            if PEST_ON==1
                All.stim_con                = 0;
                PEST_hnd_or_eye='eye';
        ...
        case 'Learning memory lini'
            SETTINGS.check_motion_jaw       = 1;
            force_conditions                = 2; % back to the pool, unsuccessful target would reappear at some point
            N_repetitions                   = 10; % how many time each condition will appear
            task.reward.time_neutral        = [0.1 0.1]; % 0.6s -> 0.2ml per hit
            task.rest_hand                  = [1 1];
            ...
            All.color_con                   = [3]; % 1 bright target, 2 dim learning target
            All.reach_hand_con              = [1]; % 1 left, 2 right
            All.type_con                    = [2.5]; % 2.5 learning memory, 2 direct
            All.effector_con                = 4;
            All.timing_con                  = 33; % 32 learning memory, 2 direct
            All.size_con                    = 2; % 2 reach sizes, 3 for bigger targets
            All.instructed_choice_con       = [0]; % 0 instructed, 1 choice
            All.excentricities              = [12,24]; % either 12 or [12 24]
            All.angle_cases                 = [1,2,3,4,5,6]; % which targets to present depending on pool of angles
            All.stim_con                    = [0];
            if PEST_ON==1
        ...
        case 'Memory lini'
            SETTINGS.check_motion_jaw       = 1;
            force_conditions                = 1; % back to the pool, unsuccessful target would reappear at some point
            N_repetitions                   = 5; % how many time each condition will appear
            task.reward.time_neutral        = [0.11 0.11]; % 0.6s -> 0.2ml per hit
            task.rest_hand                  = [1 1];
            ...
            All.color_con                   = [4]; % 1 bright target, 2 dim learning target 4 diss saccades
            All.reach_hand_con              = [1 2]; % 1 left, 2 right
            All.type_con                    = 3; % 2.5 learning memory, 2 direct 3 memory
            All.effector_con                = 3; % diss sacc 
            All.timing_con                  = 33; % 32 learning memory, 2 direct 33 memory
            All.size_con                    = 2; % 2 reach sizes, 3 for bigger targets
            All.instructed_choice_con       = [0 ]; % 0 instructed, 1 choice
            All.excentricities              = [12 24]; % either 12 or [12 24]
            All.angle_cases                 = [1 2 3 4 5 6]; % which targets to present depending on pool of angles
        ...
        case 'Direct saccades flaffi'
            SETTINGS.check_motion_jaw       = 0;
            force_conditions                = 2; % back to the pool, unsuccessful target would reappear at some point
            N_repetitions                   = 20; % how many time each condition will appear
            task.reward.time_neutral        = [0.25 0.25];
            task.rest_hand                  = [0 0];
            ...
            All.color_con                   = [1];
            All.reach_hand_con              = [1]; % 1 left, 2 right
            All.type_con                    = 2; % 2.5 learning memory, 2 direct
            All.effector_con                = 0;
            All.timing_con                  = 40; %<----------
            All.size_con                    = 2;
            All.instructed_choice_con       = [0 1]; % 0 instructed, 1 choice
        ...
        case 'Direct joint movements'
            ...
            All.instructed_choice_con       = [0 1]; % 0 instructed, 1 choice
            All.excentricities              = [12 24]; % either 12 or [12 24]
            All.angle_cases                 = [1:6]; % which targets to present depending on pool of angles
            All.stim_con                    = [0];
            if PEST_ON==1
                All.stim_con                = 0;
                PEST_hnd_or_eye='eye';
                PEST_effector=1;
                 ...
                 case 'Direct dissociated saccades flaffy'
        ...
        case 'Memory saccades flaffi' % Also for memory training saccades
            SETTINGS.check_motion_jaw       = 0;
            force_conditions                = 2; % back to the pool, unsuccessful target would reappear at some point
            N_repetitions                   = 20; % how many time each condition will appear
            task.reward.time_neutral        = [0.25 0.25]; % 0.6s -> 0.2ml per hit
            ...
            All.type_con                    = 3;  %<---------- or 2.5 for memory learning
            All.effector_con                = 0;
            All.timing_con                  = 40; %<---------- modify here the cue and memory times
            All.size_con                    = 40; %<---------- modify here the radius and window size
            All.instructed_choice_con       = [0 1]; % 0 instructed, 1 choice
            All.excentricities              = [12 24]; % either 12 or [12 24]
            All.angle_cases                 = [1 2 3 4 5 6]; % which targets to present depending on pool of angles
            All.stim_con                    = [0];
...
switch Current_con.timing_con
    ...
    case 1 %'fixation'
        ...
        task.timing.ITI_fail_var                = 0;
        task.timing.grace_time_eye              = 0.02;
        task.timing.grace_time_hand             = 0;
        task.timing.fix_time_to_acquire_hnd     = 1;
        task.timing.tar_time_to_acquire_hnd     = 1.5;
    ...
    case 2 %'direct'
        ...
        task.timing.grace_time_eye              = 0;
        task.timing.grace_time_hand             = 0;
        task.timing.fix_time_to_acquire_hnd     = 2.5;
        task.timing.tar_time_to_acquire_hnd     = 2.5;
        task.timing.tar_inv_time_to_acquire_hnd = 2;
        task.timing.fix_time_to_acquire_eye     = 0.5;
        task.timing.tar_time_to_acquire_eye     = 0.5;
        task.timing.tar_inv_time_to_acquire_eye = 0.5; %3
    ...
    case 40 %'memory play flaffi'
        ...
        task.timing.fix_time_hold_var           = 0.3;
        task.timing.cue_time_hold               = 0.3; %<-----------
        task.timing.cue_time_hold_var           = 0.2; %<-----------
        task.timing.mem_time_hold               = 0.4; %<-----------
        task.timing.mem_time_hold_var           = 0.6; %<-----------
        task.timing.del_time_hold               = 0;
        task.timing.del_time_hold_var           = 0;
        task.timing.tar_inv_time_hold           = 0.2; %<-----------
    ...
    case 33 %'memory learning lini'
        ...
        task.timing.mem_time_hold_var           = 0.6;
        task.timing.del_time_hold               = 0.4;
        task.timing.del_time_hold_var           = 0.6;
        task.timing.tar_inv_time_hold           = 0.2;
        task.timing.tar_inv_time_hold_var       = 0.1;
        task.timing.tar_time_hold               = 0.1;
        task.timing.tar_time_hold_var           = 0;
...
switch Current_con.color_con
    case 1 % normal dim bright targets, greyish cues
    ...
    case 2 % normal cues, dimmer dim and bright targets for 2.5 saccades and reach tasks
        task.eye.fix.color_dim       = [128 0 0]; %
        task.eye.fix.color_bright    = [255 0 0];
        task.eye.tar(1).color_dim    = [4 0 0];  % 2.5 or 3
        task.eye.tar(1).color_bright = [10 0 0];
        task.eye.tar(2).color_dim    = [4 0 0]; %  % 2.5 or 3
        task.eye.tar(2).color_bright = [10 0 0];
        
        task.hnd_right.color_dim    = [0 3 0]; %
        task.hnd_right.color_bright = [0 6 0];
        task.hnd_right.color_cue    = [50 80 50];
        task.hnd_left.color_dim     = [2 3 4]; %
        task.hnd_left.color_bright  = [4 6 8];
        task.hnd_left.color_cue     = [64 77 80];
        task.hnd_stay.color_dim     = [128 129 0];
        task.hnd_stay.color_bright  = [255 255 0];
        
    case 3 % normal cues, dimmer dim and bright targets for 2.5 reach tasks
        task.eye.fix.color_dim       = [128 0 0]; %
        task.eye.fix.color_bright    = [255 0 0];
        task.eye.tar(1).color_dim    = [128 0 0];  % 2.5 or 3
        task.eye.tar(1).color_bright = [255 0 0];
    ...
    case 4 % normal dim bright targets, greyish cues
        task.eye.fix.color_dim       = [128 0 0]; %
        task.eye.fix.color_bright    = [255 0 0];
        task.eye.tar(1).color_dim    = [128 0 0];  % 2.5 or 3
        task.eye.tar(1).color_bright = [255 0 0];
        ...
        task.hnd_left.color_dim     = [39 109 216]; %
        task.hnd_left.color_bright  = [119 230 253];
        task.hnd_left.color_cue     = [119 230 253];
        task.hnd_stay.color_dim     = [128 129 0];
        task.hnd_stay.color_bright  = [255 255 0];
