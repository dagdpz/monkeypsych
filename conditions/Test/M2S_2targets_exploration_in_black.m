...
%experiment='Match to sample saccades random locations';
    ...
    switch experiment
        ...
        case 'Match to sample saccades'
            ...
            N_repetitions                   = 4;        % 1 repetition -> 96 trials
            task.rest_hand                  = [0 0];
            ...
            ex_BT                 = 7.5;
            All.excentricities    = [10,20]; % & 20! the eccentricity is randomly chosen to be 12 or 20
            ...
            All.angle_cases                 = [1];
            All.tar_dis_con                 = 0; 
            All.mat_dis_con                 = 1:4;      % triangle pointing randomly left or right
            All.cue_pos_con                 = 1:2;      % right and left cue
            All.shape_con                   = 1:8;    % target and distractor distribution
            
            % THINGS TO CHANGE
            adjusted_curvature              = -0.7;      % change curvature here
    end
...
switch Current_con.timing_con
    ...
    case 5 %'Match to sample'
        ...
        task.timing.tar_time_to_acquire_hnd     = 20;
        task.timing.tar_inv_time_to_acquire_hnd = 20;        
        task.timing.fix_time_to_acquire_eye     = 2;
        task.timing.tar_time_to_acquire_eye     = 20;  % time to explore the black screen; 12 for the experiment
        task.timing.tar_inv_time_to_acquire_eye = 20;  
        task.timing.fix_time_hold               = 0.2; % fixation time
        task.timing.fix_time_hold_var           = 0.2;
        task.timing.cue_time_hold               = 0.8; % cue time
...
left_top  = [fix_eye_x-ex_LR,fix_eye_y+ex_BT];
left_bot  = [fix_eye_x-ex_LR,fix_eye_y-ex_BT];
right_top = [fix_eye_x+ex_LR,fix_eye_y+ex_BT];
right_bot = [fix_eye_x+ex_LR,fix_eye_y-ex_BT];
...
% all_curv=[-0.7,-0.4,0.4,0.7];
% all_sides={'LR','LR','LR','LR'};

switch Current_con.shape_con

    case 1
        task.convexities=num2cell([all_curv(1),all_curv(2)]);
        convex_sides=[all_sides(1),all_sides(2)];
        distractor_convexity = all_curv(4);
        distractor_side = all_sides{4};      
    case 2
        task.convexities=num2cell([all_curv(2),all_curv(1)]);
        convex_sides=[all_sides(2),all_sides(1)];
        distractor_convexity = all_curv(4);
        distractor_side = all_sides{4};   
    case 3
        task.convexities=num2cell([all_curv(2),all_curv(3)]);
        convex_sides=[all_sides(2),all_sides(3)];
        distractor_convexity = all_curv(4);
        distractor_side = all_sides{4};   
    case 4
        task.convexities=num2cell([all_curv(3),all_curv(2)]);
        convex_sides=[all_sides(3),all_sides(2)];
        distractor_convexity = all_curv(4);
        distractor_side = all_sides{4};   
    case 5
        task.convexities=num2cell([all_curv(3),all_curv(4)]);
        convex_sides=[all_sides(3),all_sides(4)];
        distractor_convexity = all_curv(2);
        distractor_side = all_sides{2};  
    case 6
        task.convexities=num2cell([all_curv(4),all_curv(3)]);
        convex_sides=[all_sides(4),all_sides(3)];
        distractor_convexity = all_curv(2);
        distractor_side = all_sides{2};  
    case 7
        task.convexities=num2cell([all_curv(1),all_curv(4)]);
        convex_sides=[all_sides(1),all_sides(4)];
        distractor_convexity = all_curv(2);
        distractor_side = all_sides{2};  
    case 8
        task.convexities=num2cell([all_curv(4),all_curv(1)]);
        convex_sides=[all_sides(4),all_sides(1)];
        distractor_convexity = all_curv(2);
        distractor_side = all_sides{2};  
    case 9
        task.convexities=num2cell([all_curv(1),all_curv(2)]);
...
switch Current_con.mat_dis_con
    case 1
        task.tar_pos = [{left_top},  {right_top}];
    case 2
        task.tar_pos = [{left_bot},  {right_bot}];
    case 3
        task.tar_pos = [{right_top},  {left_top}];
    case 4
        task.tar_pos = [{right_bot},  {left_bot}];
end
...
switch Current_con.cue_pos_con
    case 1
        task.cue_pos  = {[-ex_cue,fix_eye_y],[ex_cue,fix_eye_y]}; 
    case 2
        task.cue_pos  = {[ex_cue,fix_eye_y],[-ex_cue,fix_eye_y]};
end
