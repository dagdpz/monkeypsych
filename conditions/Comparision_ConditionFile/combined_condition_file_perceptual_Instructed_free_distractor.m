...
%         'choice distractor-distractor memory saccades diagonal','choice distractor-distractor memory saccades horizontal'};
    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        reward_memory                       = 0.14;
        reward_direct                       = 0.16; %0.14;
        
        colors_tardist                      = [1 8 ];  % [1 8]  psychphysic curve: [1 5:8] target - distractor
        colors_distdist                     = [2 12 ]; % [2 12] [2 9:12] distractor - distractor
        colors_dist                         = [4 16 ]; % [4 16] [4 13:16]; single distractor
...
task.timing.ITI_fail                = 2; %1 changed 20190708
task.timing.ITI_fail_var            = 0;
task.timing.ITI_incorrect_completed = 2; %1
...
task.eye.fix.color_bright       = [255 0 0]; % [255 0 0]
       ...
       Distr_easy_color_dim =   [60 60 0];
       Distr_easy_color_bright = [123 123 0];
         
       Distr_diff1_color_dim = [95 59 0];
       Distr_diff1_color_bright = [185 90 0];

       Distr_diff2_color_dim = [113 42 0];
       Distr_diff2_color_bright = [205 60 0];
       

       Distr_diff3_color_dim = [128 23 0];
       Distr_diff3_color_bright = [228 20 0];     
...
%         task.eye.tar(2).color_bright    = [255 0 0];        
    ...
    case 5 % choice target-distractor saccades
        ...
        task.eye.tar(1).color_dim       = [128 0 0]; % red target
        task.eye.tar(1).color_bright    = [255 0 0];
        task.eye.tar(2).color_dim       = [95 59 0]; % distractor %95 59 0 %Corny
        task.eye.tar(2).color_bright    = [185 90 0]; %185 90 0 %Cornz 220 40 0
        task.eye.tar(3).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(3).color_bright    = [110 110 110];
    ...
    case 6 
        ...
        task.eye.tar(1).color_dim       = [128 0 0]; % red target
        task.eye.tar(1).color_bright    = [255 0 0];
        task.eye.tar(2).color_dim       = [113 42 0]; % distractor %113 42 0
        task.eye.tar(2).color_bright    = [205 60 0]; %205 60 0
        task.eye.tar(3).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(3).color_bright    = [110 110 110];
    ...
    case 8 
        ...
        task.eye.tar(1).color_dim       = [128 0 0]; % red target
        task.eye.tar(1).color_bright    = [255 0 0];
        task.eye.tar(2).color_dim       = Distr_diff3_color_dim; % distractor for Curius [128 18 0]  %% distractor for Cornelius [128 11 0]
        task.eye.tar(2).color_bright    = Distr_diff3_color_bright;       
        task.eye.tar(3).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(3).color_bright    = [110 110 110];
    ...
    case 9 % choice distractor-distractor saccades
        ...
        task.eye.tar(1).color_dim       = [95 59 0]; % distractor [126 30 0]
        task.eye.tar(1).color_bright    = [185 90 0]; %[220 40 0]
        task.eye.tar(2).color_dim       = [95 59 0]; % distractor %95 59 0
        task.eye.tar(2).color_bright    = [185 90 0]; %185 90 0
        task.eye.tar(3).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(3).color_bright    = [110 110 110];
    ...
    case 11 
        ...
        task.eye.tar(2).color_dim       = [128 23 0]; % distractor %126 30 0
        task.eye.tar(2).color_bright    = [228 20 0]; %220 40 0
        task.eye.tar(3).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(3).color_bright    = [110 110 110];
        
    case 12
        task.eye.tar(1).color_dim       = Distr_diff3_color_dim; % distractor for Curius [128 18 0]  %% distractor for Cornelius [128 11 0]
        task.eye.tar(1).color_bright    = Distr_diff3_color_bright;
        task.eye.tar(2).color_dim       = Distr_diff3_color_dim; % % distractor for Curius[128 18 0]  %% distractor for Cornelius [128 11 0]
        task.eye.tar(2).color_bright    = Distr_diff3_color_bright; % 228 20 0
        task.eye.tar(3).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(3).color_bright    = [110 110 110];
    ...
    case 14
        ...
        task.eye.tar(1).color_dim       = [113 42 0]; % distractor 113 42 0
        task.eye.tar(1).color_bright    = [205 60 0]; %205 60 0
        task.eye.tar(2).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(2).color_bright    = [110 110 110];
    ...
    case 15 
        ...
        task.eye.tar(1).color_dim       = [128 23 0]; % 
        task.eye.tar(1).color_bright    = [228 20 0];
        task.eye.tar(2).color_dim       = [60 60 60]; % fixation target
        task.eye.tar(2).color_bright    = [110 110 110];
...
switch Current_con.colors_con
    ...
    case 15
        ...
        task.eye.cue(1).color_dim       = [128 11 0];
        task.eye.cue(1).color_bright    = [128 11 0]; %126 30 0
        task.eye.cue(2).color_dim       = [110 110 110];
        task.eye.cue(2).color_bright    = [110 110 110];
