...
%%
    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        All.stim_con                        = 0;
        All.reach_hand_con                  = 1; % 2=right 1= links, [1,2] = alterniered
        ...
        All.Percentage_CueAuditiv               = 0.1;% percentage in 10 trials how often a tone (1) should be displayed
     ...
     % Match-to-sample stimulus: rotation defines the sample & rotationDifference defines the distractor/NonSample   
        ...
        task.PayoffMatrix = [0.07 0.1 0.2; 0.14 0.1 10]; 
    ...
    if  n_exp == 0 % pseudo-randomization for fMRI tasks
        ...
        % forward & backward approach for the choice of the non-match %  sequence_matrix([20,23],:)
        ...
        if All.type_con == 9 || All.type_con == 10
            ...
            for ind_sequence_matrix = 1: size(sequence_matrix,2)
                if (sequence_matrix(IndRotation,ind_sequence_matrix) + sequence_matrix(IndRotationDiff,ind_sequence_matrix))> 50
        ...
        if All.type_con == 10
            ...
            En = 10;
            St = 1;
            for ind_sequence_matrix = 1: size(sequence_matrix,2)/10
                sequence_matrix(IndCueAuditiv,St:En) = randsample([ones(round((All.Percentage_CueAuditiv)*10),1)', zeros(round((1-All.Percentage_CueAuditiv)*10),1)'], 10);
...
task.timing.fi2_time_to_acquire_hnd = 2; 
task.timing.fi2_time_to_acquire_eye = 2; % 0.5
...
task.timing.msk_time_hold=1; %MASK
...
switch Current_con.timing_con
    ...
    case 1 % choice 
        ...
        task.timing.fi2_time_hold               = 0.2; % duration of initial fixation
        ...
        task.timing.tar_time_to_acquire_hnd     = 2; %0.6
        ...
        task.timing.cue_auditiv_time_hold_var   = 0.2;
        task.timing.cue_auditiv_time_hold       = 0.5;
        task.timing.ta2_time_to_acquire_eye     = 1;
        task.timing.ta2_time_to_acquire_hnd     = 3;
        task.timing.ta2_time_hold               = 0.5;
        task.timing.ta2_time_hold_var           = 0.2;
end

%% SHAPES of fixation spot and targets
task.eye.fix.shape                  = 'circle'; % 'circle', 'square'
task.hnd.fix.shape                  = 'circle'; % 'circle', 'square'
task.eye.fi2.shape                  = 'circle'; % 'circle', 'square'
task.hnd.fi2.shape                  = 'circle'; % 'circle', 'square'
...
task.hnd.ta2(1).shape.mode                = 'circle_withBar';
task.hnd.ta2(2).shape.mode                = 'circle_withBar'; 
task.hnd.ta2(3).shape.mode                = 'circle_withBar'; 
task.eye.ta2(1).shape.mode                = 'circle_withBar';
task.eye.ta2(2).shape.mode                = 'circle_withBar';
task.eye.ta2(3).shape.mode                = 'circle_withBar';
...
switch Current_con.size_con
    ...
    case 2 
        ...
        task.eye.fi2.size       = 0.5;
        task.eye.fi2.radius     = 50;
        ...
        task.hnd.fix.radius     = 2.5;
        task.hnd.fi2.size       = 2.5;
        task.hnd.fi2.radius     = 2.5;
        task.hnd.cuA(1).size    = 0;  
        task.hnd.cuA(1).radius  = 0; 
        task.hnd.tar(1).size    = 2.5;
        task.hnd.tar(1).radius  = 3;   
        task.hnd.ta2(1).size    = 3;
        task.hnd.ta2(1).radius  = 3;  
end
...
task.eye.fix.color_dim              = [60 60 60]; 
task.eye.fix.color_bright           = [60 60 60]; 

task.hnd_right.color_dim_fix         = [60 60 60];
task.hnd_left.color_dim_fix          = [60 60 60];

task.hnd_stay.color_dim_fix         = [0 0 0]; 
task.hnd_stay.color_bright_fix      = [0 0 0];
task.hnd_left.color_dim_fix         = [60 60 60];
...
task.hnd.fi2.color_dim              = [60 60 60]; 
task.hnd.fi2.color_bright           = [60 60 60]; 
...
if task.type==1 % fixation   
    ...
    task.hnd.fi2.x    = fix_hnd_x; 
    task.hnd.fi2.y    = fix_hnd_y - 10;
    task.eye.fi2.x    = fix_hnd_x; 
    task.eye.fi2.y    = fix_hnd_y - 10;
end
%% TARGET POSITIONS
...
task.hnd.ta2(1).y    = fix_hnd_y  + tar_dis_2y;         
task.hnd.ta2(2).x    = fix_hnd_x; 
task.hnd.ta2(2).y    = fix_hnd_y;
task.hnd.ta2(3).x    = fix_hnd_x  + tar_dis_1x; 
task.hnd.ta2(3).y    = fix_hnd_y  + tar_dis_1y;     

task.eye.ta2(1).x    = fix_hnd_x  + tar_dis_2x;
task.eye.ta2(1).y    = fix_hnd_y  + tar_dis_2y;         
task.eye.ta2(2).x    = fix_hnd_x; 
task.eye.ta2(2).y    = fix_hnd_y;
task.eye.ta2(3).x    = fix_hnd_x  + tar_dis_1x; 
task.eye.ta2(3).y    = fix_hnd_y  + tar_dis_1y;  
