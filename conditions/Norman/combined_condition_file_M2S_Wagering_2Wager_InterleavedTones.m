...
if ~exist('dyn','var') || dyn.trialNumber == 1
    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        All = struct(...
            ...
            'differenceBetweenSamples', NaN,'rotations', NaN,'difficultyLevel_rotation',NaN, 'target_Right',NaN, 'CueOnAuditiv',NaN, ...
            'Percentage_CueAuditiv', NaN);
        %% Tasks
        % general settings
        ...
        All.Percentage_CueAuditiv             = 0; %0.9;% percentage in 10 trials how often a tone (1) should be displayed
        ...
        difficultyLevels                    = [5 4 3 2 1]; %7 4 3 
       ...
       %  All.Nr_targets2                    = 1;%How many wagers
        %task.PayoffMatrix                   = [0 ; 0.5 ];
        
        All.Nr_targets2                    = 2;%How many wagers       
...
if  n_exp == 0 % pseudo-randomization for fMRI tasks
    ...
    %% Auditive Cue... every Number of samles
    if All.type_con == 10
...
end % end for what in the first trial happends
...
switch Current_con.timing_con
    ...
    case 1 % choice
        ...
        task.timing.tar_time_to_acquire_hnd     = 0.8; %0.9
...
switch Current_con.size_con
    ...
    case 2
        ...
        task.eye.fi2(1).size       = 0;
        task.eye.fi2(1).radius     = 50;
        
        task.eye.cue(1).size    = 0;
...
% 
% task.hnd.tar(1).ringColor             = [0 0 0];
% task.hnd.tar(2).ringColor             = [0 0 0];
% task.hnd.cue(1).ringColor             = [0 0 0];
...
%% if a tone or no tone is presented

     if  All.Percentage_CueAuditiv < 1
         if task.CueAuditiv == 0
             if Current_con.Nr_targets2 == 2
            task.PayoffMatrix                   = [0 ,0.3; 0.29 , -5];

             else 
           task.PayoffMatrix                   = [0 ,0.18 , 0.3; 0.29 ,0.15 , -5];
             end
         elseif task.CueAuditiv == 1          
            task.PayoffMatrix                   = [0 , 0.22; 0.21 , -5];
         else
             task.PayoffMatrix                   = [0 , 0.22; 0.21 , -5];
         end
     else
          task.PayoffMatrix                   = [0 , 0.25; 0.24 , -5];
     end
       
     
switch Current_con.Nr_targets2
    case 1
        task.hnd.ta2(3) = [];
         task.hnd.ta2(2) = [];
case 2 % Wager 1 & Wager 2 % Wager 3 are presented
task.hnd.ta2(2) = [];
    case 3 % Wager 1 & Wager 2 % Wager 3 are presented
        if task.CueAuditiv == 1
         task.hnd.ta2(2) = [];
        else
        end
end
