...
task.hnd.fix.size   = 0.5;
task.hnd.tar(1).radius = 5;  % deg
task.hnd.tar(2).radius = 5;  % deg
task.hnd.tar(1).size = 0.5;  % deg
task.hnd.tar(2).size = 0.5;  % deg
...
switch custom_trial_condition
    ...
    case num2cell([4:N_stim_conditions:N_total_conditions]) % instructed direct in hold
        ...
        task.microstim.end{1}      = [-0];
    case num2cell([5:N_stim_conditions:N_total_conditions]) % instructed direct in hold
        task.type = 2;
        task.choice=0;
        task.microstim.stim_on = 1; % from 0 to 1
        task.microstim.state = [STATE.FIX_HOL];
    ...
    case num2cell([9:N_stim_conditions:N_total_conditions]) % instructed direct in acquisition
        task.type = 2;
        task.choice=0;
        task.microstim.stim_on = 1; % from 0 to 1
        task.microstim.state = [STATE.TAR_ACQ];
    ...
    case num2cell([10:N_stim_conditions:N_total_conditions]) % choice baseline
        task.type = 2;
        task.choice=1;
        task.microstim.stim_on = 0; % from 0 to 1
        task.microstim.state = [STATE.FIX_HOL];
    ...
    case num2cell([12:N_stim_conditions:N_total_conditions]) % choice direct in hold
        ...
        task.microstim.start{1}    = [-0.15] ;
        task.microstim.end{1}      = [-0];        
    case num2cell([13:N_stim_conditions:N_total_conditions]) % choice direct in hold
        task.type = 2;
        task.choice=1;
        task.microstim.stim_on = 1; % from 0 to 1
        task.microstim.state = [STATE.FIX_HOL];
        task.microstim.start{1}    = [-0.1] ;
        task.microstim.end{1}      = [-0];
     case num2cell([14:N_stim_conditions:N_total_conditions]) % choice direct in hold
        task.type = 2;
        task.choice=1;
        task.microstim.stim_on = 1; % from 0 to 1
        task.microstim.state = [STATE.FIX_HOL];
    ...
    case num2cell([16:N_stim_conditions:N_total_conditions]) % choice direct in acquisition
        ...
        task.microstim.start{1}    = [0.05] ;
        task.microstim.end{1}      = [0.25];
    case num2cell([17:N_stim_conditions:N_total_conditions]) % choice direct in acquisition
        task.type = 2;
        task.choice=1;
        task.microstim.stim_on = 1; % from 0 to 1
        task.microstim.state = [STATE.TAR_ACQ];
        task.microstim.start{1}    = [0.1] ;
        task.microstim.end{1}      = [0.3];
    case num2cell([18:N_stim_conditions:N_total_conditions]) % choice direct in acquisition
        task.type = 2;
        task.choice=1;
        task.microstim.stim_on = 1; % from 0 to 1
        task.microstim.state = [STATE.TAR_ACQ];
