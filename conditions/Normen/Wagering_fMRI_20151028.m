function Wagering_fMRI_20151028(name_subject, run_number, gain)
...
path_images_rot                         = 'D:\Sources\MATLAB\Wagering\images\Size120_rotation4\';
% SETUP
...
SETTINGS.proportion_wager                = [1 1 1 1 2];% [pre-wagering(2) and post-wagering (1)]
...
%EFFECTOR
SETTINGS.button_or_sensor                ='buttons'; % 'buttons' or 'sensors'
...
switch SETTINGS.run_number
 ...
 %% delay period from 6 -8 seconds for delaym2s & Wagering   
%     case 1
%         SETTINGS.family_matrix                      = SETTINGS.family_matrix(2:end) ;
%         SETTINGS.proportion_delay_afterfixation     = [4 5 3 4 3 5 4 4 4 3 5 5 5 4 3 4 4 3 3 3 5 4 3 5 5 4 4 4 5];
%         SETTINGS.proportion_delay_afterMatch        = [8 8 6 8 8 8 6 8 7 8 6 7 8 8 8 8 7 7 6 6 8 6 6 7 7 7 8 6 7];
%         SETTINGS.proportion_delay_afterPostWag      = [8 8 8 8 8 7 7 7 8 7 6 7 8 7 8 7 8 8 8 8 8 7 8 8 7 8 8 8 6];
%     case 2
%         SETTINGS.proportion_delay_afterfixation     = [4 4 5 4 5 3 3 4 3 4 5 3 3 3 5 4 4 3 3 4 4 3 4 5 5 4 3 5 3 4];
%         SETTINGS.proportion_delay_afterMatch        = [6 6 7 7 7 6 6 7 8 8 8 6 8 8 8 6 7 8 8 6 7 7 7 8 6 8 8 7 6 8];
%         SETTINGS.proportion_delay_afterPostWag      = [8 8 7 7 8 8 8 6 7 8 8 8 6 8 8 8 8 8 8 8 7 8 8 8 7 6 8 7 8 6];
%     case 3
%         SETTINGS.proportion_delay_afterfixation     = [4 5 3 3 4 4 4 4 3 3 3 3 3 4 5 3 5 4 3 3 3 5 5 5 4 4 5 3 3 5];
%         SETTINGS.proportion_delay_afterMatch        = [6 7 7 8 7 6 7 7 7 6 7 6 8 7 7 8 7 6 6 7 6 8 8 8 8 7 6 6 7 7];
%         SETTINGS.proportion_delay_afterPostWag      = [8 8 8 7 8 8 6 8 6 8 8 8 7 8 8 8 8 8 8 7 7 8 8 7 7 7 8 8 8 8];
%     case 4
%         SETTINGS.proportion_delay_afterfixation     = [5 3 5 3 5 5 4 5 4 3 5 3 3 3 3 4 4 4 3 4 4 3 3 5 3 3 5 5 5 4];
%         SETTINGS.proportion_delay_afterMatch        = [7 7 8 7 6 8 7 6 8 7 6 6 8 8 8 8 8 8 6 6 8 7 7 6 6 6 8 6 7 7];
%         SETTINGS.proportion_delay_afterPostWag      = [8 6 7 8 8 8 7 6 8 8 8 6 8 8 8 8 6 8 8 7 7 8 8 8 8 8 7 8 6 6];
end
...
SETTINGS.dist_extra_right                = 120; % with this we have in psychophysics PC an excentricity of 9 degree
...
%     load([path_images_rot 'gray_withoutbar.jpg']),
%     load([path_images_rot 'family_1.jpg']),
    %     CirWithoutBar = imread([path_images 'gray_withoutbar.jpg']);
    ...
    %
    %
    %     CircleWithoutBar = imresize(CirWithoutBar,[SETTINGS.size_images SETTINGS.size_images]);
    ...
    %     save  CircleWithoutBar.mat CircleWithoutBar
    %     save planet10.mat planet10
    ...
    if rest_sensor_hold_success, % continue to target hand input
        ...
        if SETTINGS.present_mask
            [time_mask] = mask(SETTINGS.number_of_rects);
        end
        ...
        if subject_got_sample,
            last_event_time  = GetSecs;
             time_record_BOLD = SETTINGS.time_process_sample-time_mask;
        ...
        if ~unsuccess
            ...
            while true
                ...
                if (time_match_to_sample_selected - time_matchtosample_appeared < SETTINGS.time_present_match_to_sample)...
                        && ~no_bar && match_to_sample_selected % bar disappears & visual feedback: not in resting position
                    Screen('FrameRect', SETTINGS.window, SETTINGS.gray_cue_color,dyn_wager.position_rest,10);
                    Screen('FillOval', SETTINGS.window, SETTINGS.red_fix_bright1,dyn_wager.position_fixation);
                ...
                if get_hands_state == SETTINGS.rest_buttons  && no_bar == 1 && match_to_sample_selected
                    Screen('FillRect', SETTINGS.window, SETTINGS.gray_cue_color,dyn_wager.position_rest);
                    Screen('FillOval', SETTINGS.window, SETTINGS.red_fix_bright1,dyn_wager.position_fixation);
                    Screen('DrawTexture',SETTINGS.window,SETTINGS.imgWithoutBar(1),[0 0 SETTINGS.size_images SETTINGS.size_images], position_msample);
                    Screen('DrawTexture',SETTINGS.window,SETTINGS.imgWithoutBar(1),[0 0 SETTINGS.size_images SETTINGS.size_images], position_nonmsample);
                ...
                if GetSecs - time_match_to_sample_selected > SETTINGS.time_back_match_to_sample && get_hands_state ~= SETTINGS.rest_buttons ; % this will never be fulfilled
                ...
                if (GetSecs - time_matchtosample_appeared > SETTINGS.time_present_match_to_sample)...
                        && match_to_sample_selected; % after 1.5 sek break
                    time_match_to_sample_end = GetSecs;
                    break;
                end
                if (GetSecs - time_matchtosample_appeared > SETTINGS.time_present_match_to_sample) && ~match_to_sample_selected;
                    [unsuccess] = failure1; %too slow
                    time_match_to_sample_end = GetSecs;
                    break;
                end
            ...
            while 1
                if get_hands_state == SETTINGS.no_button
                    [unsuccess] = failure3; % both button were released after match-to-sample
                    break;
        ...
        if ~unsuccess
            last_event_time = GetSecs;
            %time_record_BOLD = SETTINGS.time_process_match_to_sample + randi([0,2],1); % NEU randomisied delay
        ...
        if SETTINGS.wager
            ...
            while ~unsuccess
                ...
                if get_hands_state ~= SETTINGS.rest_buttons
                    [Wager_time_decided_first_click , Wager_number_yellow, Amount_clicks , Wager_time_decided_highvslow ,wager_choosen ,value_choosen ,unsuccess ,amount_rectangular ,decision_left ,wager_control_completed, Wager_time_decided] = wager(Wager_time_decided_first_click , Wager_number_yellow, Amount_clicks, Wager_time_decided,wager_choosen,low_wager_right,value_choosen,Wager_start_time,eye_offset_x,eye_offset_y,allow_grace_time);
                    break;
                end
            end
    ...
    trial(Trial).time_mask                                 = time_mask;
...
postwaging_rot_deg_matrix = Shuffle(repmat(SETTINGS.rot_deg_prop,1,4)); % 25 trials post-wagering
...
matrix_family_pre                         = Shuffle(repmat(SETTINGS.proportion_wager,1,5));
...
function [Wager_time_decided_first_click, Wager_number_yellow ,Amount_clicks, Wager_time_decided_highvslow ,wager_choosen ,value_choosen ,unsuccess ,amount_rectangular ,decision_left, wager_control_completed ,Wager_time_decided] = wager(Wager_time_decided_first_click,  Wager_number_yellow ,Amount_clicks,Wager_time_decided,wager_choosen,low_wager_right,value_choosen,Wager_start_time,eye_offset_x,eye_offset_y,allow_grace_time)
...
while true
    ...
    if Amount_clicks == SETTINGS.number_of_allowed_clicks
        ...
        WaitSecs(SETTINGS.present_visual_feedback+time);
        break;
    end
...
function [time_mask] = mask(number_of_rects)
...
rand_rect                     = Shuffle(repmat([1,2,3,3,3],1,3));%[1,1,2,2,3,3,3,3,3]); %randi([1 3],number_of_rects,1);  
...
time_mask = time_after - time_before;
