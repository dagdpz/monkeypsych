...
% esperimentazione        = {'instructed memory saccades','instructed distractor memory saccades'};
    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        pool_of_angles                      = [0,180]; %[0,30,150,180,210,330] [20,0,340,200,180,160]
...
switch Current_con.colors_con
    ...
    case 0 % choice target-target saccades
        ...
        task.hnd.tar(1).color_dim       = [128 0 0]; % red target
        task.hnd.tar(1).color_bright    = [255 0 0];         
        task.hnd.tar(2).color_dim       = task.eye.tar(1).color_dim; % red target
        task.hnd.tar(2).color_bright    = task.eye.tar(1).color_bright;
        task.hnd.tar(3).color_dim       = [60 60 60]; % fixation target
        task.hnd.tar(3).color_bright    = [110 110 110];
        % luminance test
...
%         task.eye.tar(2).color_bright    = [255 0 0];        
    ...
    case 1 % choice target-distractor saccades
        ...
        task.eye.tar(1).color_dim       = [60 60 60]; % red target
        task.eye.tar(1).color_bright    = [60 60 60];
        task.eye.tar(2).color_dim       = [60 60 60]; % yellow distractor, in setup 1 [123 123 0] same luminance as [255 0 0]
        task.eye.tar(2).color_bright    = [60 60 60];
        task.eye.tar(3).color_dim       = [128 0 0]; % fixation target
        task.eye.tar(3).color_bright    = [128 0 0];
