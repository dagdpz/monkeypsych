    ...
    for n_exp = 1:numel(esperimentazione)
        ...
        switch experiment
            ...
            case 'Delay free_gaze'
                ...
                task.reward.time_neutral            = [0.2 0.2]; % 0.2s -> 0.24ml per hit
            ...
            case 'Delay diss_saccade'
                ...
                fix_eye_y                           = -3.5;
                fix_hnd_y                           = -6.5;
                pool_of_angles                      = [20,0,340,200,180,160]; % [90,270]; %
            ...
            case 'Direct diss_reach'
                ...
                task.reward.time_neutral            = [0.5 0.5]; % 0.6s -> 0.2ml per hit
                task.rest_hand                      = [1 1];
                
                All.offset_con                      = 0;
                All.reach_hand_con                  = [1 2];
                All.type_con                        = 2;
                All.effector_con                    = 4; % 3 4 6
                All.timing_con                      = 32; %timing
                All.size_con                        = 20;
                All.instructed_choice_con           = [0 1];
                All.var_x                           = 0;
                All.var_y                           = 0;
...
switch Current_con.timing_con
    ...
    case 31 %'memory ephys'  'Delay'
        ...
        task.timing.fix_time_hold               = 0.7; % 0.5
      ...
      case 33 % Delay experiment with long ITI
        
        task.rest_sensors_ini_time              = 0.5; % s, time to hold sensor(s) in initialize_trial before trial starts
        task.timing.wait_for_reward             = 0.3;
        task.timing.ITI_success                 = 5.5; % 3 inter trial interval
        task.timing.ITI_success_var             = 0;
        task.timing.ITI_fail                    = 5.5; % 3 inter trial interval
        task.timing.ITI_fail_var                = 0;
        task.timing.grace_time_eye              = 0;
        task.timing.grace_time_hand             = 0;
        task.timing.fix_time_to_acquire_hnd     = 1.5;
        ...
        task.timing.tar_time_to_acquire_eye     = 0.5;
        task.timing.tar_inv_time_to_acquire_eye = 0.5; %3
        task.timing.fix_time_hold               = 0.7; % 0.3
        task.timing.fix_time_hold_var           = 0.5; % 0.3
...
switch Current_con.size_con
    ...
    case 2 %'del' ephys
        task.eye.fix.size       = 0.5;
        task.eye.fix.radius     = 6.0;
        task.eye.tar(1).size    = 0.5;
        task.eye.tar(1).radius  = 6.0;
