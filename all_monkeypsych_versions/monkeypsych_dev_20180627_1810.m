...
while true
    ...
    switch dyn.state
        ...
        case STATE.INI_TRI
            ...
            task.eye.ta2(1).x   =condition(4);%KK
            task.eye.ta2(1).y   =condition(5);%KK
            task.eye.ta2(2).x   =condition(6);%KK
            task.eye.ta2(2).y   =condition(7);%KK
            ...
            task.hnd.ta2(1).x   =condition(12); %KK
            task.hnd.ta2(1).y   =condition(13); %KK
            task.hnd.ta2(2).x   =condition(12); %KK
            task.hnd.ta2(2).y   =condition(13); %KK
            ...
            task.eye.cuA(1).y   =condition(7);
            task.eye.cuA(2).y   =condition(7);
            ...
            switch task.reach_hand
                ...
                case 1 % left
                    ...
                    task.hnd.fi2.color_dim      =task.hnd_left.color_dim_fi2;
            ...
            if any(task.rest_hand)
                ...
                if SETTINGS.VisFeedback_rest_hand %KK
                    gray_cue_color                  = [60 60 60];
                    position.rest_Hand         = [SETTINGS.screen_w_pix/2-40 SETTINGS.screen_h_pix/3-40 SETTINGS.screen_w_pix/2+40 SETTINGS.screen_h_pix/3+40];
                    Screen('FrameRect', SETTINGS.window,gray_cue_color,position.rest_Hand,10); % a gray square on the screen
                    Screen(SETTINGS.window,'Flip');
                end
                while 1 % wait until sensors are correctly touched
                    ...
                    if mod(wait4sensors_iteration,2*(1/dyn.duration)) == 2*(1/dyn.duration)-1
                        ...
                        %                 gray_cue_color                  = [60 60 60];
                        %                 position.rest_Hand         = [SETTINGS.screen_w_pix/2-40 SETTINGS.screen_h_pix/3-40 SETTINGS.screen_w_pix/2+40 SETTINGS.screen_h_pix/3+40];
                        %                 Screen('FrameRect', SETTINGS.window,gray_cue_color,position.rest_Hand); % a gray square on the screen
                        %                 Screen(SETTINGS.window,'Flip');
                        %                 end
                        Screen(SETTINGS.window,'Flip');
                    end
        ...
        case STATE.TA2_ACQ  % target acquisition
            ...
            if all(isnan(dyn.target2_selected)) % still no target has been selected (e.g. in task.type = 2)
        ...
        case STATE.TA2_HOL %  target holding_with_change_of_color
            ...
            elseif dyn.n_eye_tar>0
                for n_obj=1:dyn.n_eye_tar
                    par_eye(n_obj) = aux_FillPar(trial(dyn.trialNumber).eye.ta2(n_obj).shape, trial(dyn.trialNumber).eye.ta2(n_obj).pos, task.eye.fix(1).color_bright, ...
        ...
        case {STATE.CUE_ON_AUDITIV}
            ...
            % TODO: display a tone dependent of different criteria (ex. performance % in % no tone)
           % if dyn.target_selected(2) == 1
        ...
        case STATE.ABORT % abort
            ...
            if task.type == 10
                % which wager selected? change the rewardValue according to
                % selected wager (all incorrect)
                trial(dyn.trialNumber).reward_time = task.reward.wager(dyn.target2_selected(dyn.ta2_selected_ind)).wrong;
                trial(dyn.trialNumber).timeout = task.timeout.wager(dyn.target2_selected(dyn.ta2_selected_ind)).wrong;
                ...
                if trial(dyn.trialNumber).timeout > 0 %TIMEOUT
                    dyn.duration=trial(dyn.trialNumber).timeout +trial(dyn.trialNumber).task.timing.wait_for_reward;
                    [success,dyn,task]=wait_while_recording_state(task,dyn);
                    trial(dyn.trialNumber).rewarded = 0;
                end
            end
        ...
        case STATE.REWARD % reward
            ...
            if task.type == 10
                % which wager selected? change the rewardValue according to
                % selected wager (all correct)
                trial(dyn.trialNumber).reward_time = task.reward.wager(dyn.target2_selected(dyn.ta2_selected_ind)).correct;
            end
            dyn.duration=trial(dyn.trialNumber).task.timing.wait_for_reward;
            [success,dyn,task]=wait_while_recording_state(task,dyn);
...
while true
        ...
        case 0 % eye
            for obj = 1:n_obj_eye
                if aux_IsWithinRadius(x_eye, y_eye, par_eye(obj).deg.x, par_eye(obj).deg.y, par_eye(obj).deg.radius),
                    ...
                    if isnan(dyn.target2_selected(1)), dyn.target2_selected(1) = obj; end
        ...
        case {1,6} % hand
            for obj = 1:n_obj_hnd
                if aux_IsWithinRadius(x_hnd, y_hnd, par_hnd(obj).deg.x, par_hnd(obj).deg.y, par_hnd(obj).deg.radius),
                    ...
                    if isnan(dyn.target2_selected(2)), dyn.target2_selected(2) = obj; end
                end
            end
            eye_acq = 1;
            ...
            for obj = 1:n_obj_hnd
                if aux_IsWithinRadius(x_hnd, y_hnd, par_hnd(obj).deg.x, par_hnd(obj).deg.y, par_hnd(obj).deg.radius),
                    ...
                    if isnan(dyn.target_selected(2)),
                        ...
                        dyn.target2_selected(2) = obj;
                        dyn.target_selected(1)  = 1; % dummy
                        dyn.target2_selected(1) = 1;
    ...
    if success,
        if  ~isnan(dyn.target_selected(dyn.tar_selected_ind)) && ...
            ...
            dyn.target_selected = [NaN NaN];
        elseif ~isnan(dyn.target2_selected(dyn.ta2_selected_ind)) && ...
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).x ==  trial(dyn.trialNumber).task.(dyn.effector).fi2.x && ...
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).y == trial(dyn.trialNumber).task.(dyn.effector).fi2.y && ...
                GetSecs - tEnterState < dyn.duration % if target_selected is fixation target and time_to_acquire_eye is not over yet
            dyn.target2_selected = [NaN NaN]; %%KK??
        else
            dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
            break;
        end
