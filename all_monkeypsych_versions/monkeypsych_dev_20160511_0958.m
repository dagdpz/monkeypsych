...
%             break;
        ...
        if  trial(dyn.trialNumber).task.eye.tar(dyn.target_selected(1)).x == (dyn.fixation_target(1)) && trial(dyn.trialNumber).task.eye.tar(dyn.target_selected(1)).y == (dyn.fixation_target(2)) && GetSecs - tEnterState >= task.timing.tar_time_to_acquire_eye
            dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
            break;
        else                
            dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
