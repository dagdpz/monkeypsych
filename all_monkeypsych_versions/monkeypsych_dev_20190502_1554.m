...
function [is_moving_it] = aux_GetMonkeyMotion
...
if strcmp(SETTINGS.Motion_detection_interface,'DAQ')
    ...
    is_moving_it = [is_moving_jaw is_moving_body];
else
    if numel(SETTINGS.sensor_pins) > 4 % scanner DPZ with additional pin for scanner trigger
        sen = double(get_sensors_state(SETTINGS.pp,SETTINGS.sensor_pins));
        is_moving_it = [sen(3) ~sen(4)]; % sen(3) = 1 if jaw is moving, sen(3) = 0 if there is no jaw motion, sen(4) always 1 because no body motion detector connected
    else
        for n_samples = 1:10000
            sen_it(n_samples,:) = double(get_sensors_state(SETTINGS.pp,SETTINGS.sensor_pins));
        end
                    is_moving_it = [sen_it(:,3) ~sen_it(:,4)]; % setups
    end
end
