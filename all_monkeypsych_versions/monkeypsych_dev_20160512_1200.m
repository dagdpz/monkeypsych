...
%             dyn.tar.HT = [dyn.HTeye dyn.HThnd];
        ...
        %% STATE.ABORT    
            ...
            if dyn.previousState==STATE.MAT_HOL || dyn.previousState==STATE.MAT_HOL_MSK || (isfield(trial(dyn.trialNumber).task, 'correct_choice_target') && dyn.previousState==STATE.TAR_HOL)
...
%         end
        ...
        if  trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).x == (dyn.fixation_target(1)) && ...
            ...
            if GetSecs - tEnterState >= dyn.duration % only select target if time_to_acquire_eye is over
                ...
                dyn.target_selected(dyn.tar_selected_ind) = NaN;
                continue
            end
        else
            dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
