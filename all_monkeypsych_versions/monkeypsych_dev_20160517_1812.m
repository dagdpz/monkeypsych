...
success         = false;
    ...
    if success,
        ...
        if  ~isnan(dyn.target_selected(dyn.tar_selected_ind)) && ...
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).x == (dyn.fixation_target(1)) && ...
                trial(dyn.trialNumber).task.(dyn.effector).tar(dyn.target_selected(dyn.tar_selected_ind)).y == (dyn.fixation_target(2)) % if target_selected is fixation target
            if GetSecs - tEnterState >= dyn.duration % only select target if time_to_acquire_eye is over
                dyn.hand_selected = find(~[sen1 sen2] & task.rest_hand);
                break
            else
