...
%             dyn.tar.HT = [dyn.HTeye dyn.HThnd];
        case STATE.TA2_ACQ  % target acquisition
            ...
            if all(isnan(dyn.target2_selected)) % still no target has been selected (e.g. in task.type = 2)
                ...
                if ~isnan(dyn.target2_selected(1))
                ...
                if ~isnan(dyn.target2_selected(2))
                    par_hnd = aux_FillPar(trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).shape, trial(dyn.trialNumber).hnd.ta2(dyn.target_selected(2)).pos, task.hnd.ta2(dyn.target_selected(2)).color_dim,...
                        trial(dyn.trialNumber).hnd.tar(dyn.target_selected(2)).ringColor, trial(dyn.trialNumber).hnd.tar(dyn.target_selected(2)).reward_prob,trial(dyn.trialNumber).hnd.tar(dyn.target_selected(2)).ringColor2);
                else
                    par_hnd = struct([]);
                end
...
elseif current_state == STATE.CLOSE,
    ...
    switch task.type
       ...
       case 10 % delayed Match-to-sample with backward masking (IN FUTURE with Wagering)
             task_states = [STATE.INI_TRI  STATE.FIX_ACQ  STATE.FIX_HOL  STATE.CUE_ON  STATE.MSK_HOL  STATE.MEM_PER...
                STATE.TAR_ACQ  STATE.TAR_HOL STATE.FI2_ACQ  STATE.FI2_HOL STATE.CUE_ON_AUDITIV STATE.TA2_ACQ  STATE.TA2_HOL  STATE.SUCCESS  STATE.REWARD  STATE.ITI];
